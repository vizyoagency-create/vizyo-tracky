...
| ~~V4~~ | ~~**Chaîne analyse-lieux de bout en bout, sur des faits.**~~ **Fait le 23/08.** Chaîne prouvée sur les DEUX lieux : `place_analyses` = 2 (Auchan rangé 07:10 le 23/08, Toulouse 09:10), `costEur = 0`, `executor = local`, `local/sonnet`, résumés spécifiques à la flotte (« 8 véhicules, 20 passages, E10, 1,998 €/L » / « 13 véhicules, 28 passages, SP95-E10 »), file vide, écran `/places` vérifié à 375 px (panneau « Analyse du lieu » signé « sonnet · 23/08 07:10 »). Au passage, **un vrai bogue du courrier trouvé et corrigé** (commit `c807674e`) : le tag psql « UPDATE 1 » se décodait comme du base64 quand le payload tombait sans padding « = » — travail orphelin en `pris`, reposé à la main puis livré en 16 s par le courrier corrigé. ⚠️ Deux restes : (1) le cron planifié de 03:10 n'a PAS tourné dans la nuit du 22 au 23 (cause inconnue, journaux perdus au déploiement — verdict au prochain 03:10, `lastRunAt` du run manuel ne le bloque pas) ; (2) le passage vérifié est d'origine `manual` — le cycle 100 % `scheduled` reste à constater une fois. Textes d'écran à rafraîchir : « Lancer maintenant… dépense réellement » (faux depuis la bascule locale) et « Désactivé par défaut ». | — | **Fait le 23/08 ~11 h** |
