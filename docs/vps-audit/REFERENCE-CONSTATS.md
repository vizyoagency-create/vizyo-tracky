...
- 📏 **Vu : 2026-09-21 — 31 / 38 SANS LIMITE MÉMOIRE (inchangé), 0 OOM 30 j, PSI 0,00 ×3, swap 256 Ko après le reboot.**
  Les 7 bornés : `tracky-postgres` 1,5 Go, `tracky-api` 1,5 Go, `tracky-redis` 384 Mo, `tracky-demo-api` 512 Mo,
  `tracky-demo-postgres` 512 Mo, `tracky-demo-redis` 128 Mo, `tracky-demo-web` 64 Mo. Mémoire 2,5 Gi utilisée / 7,8,
  5,2 Gi disponibles, `Committed_AS` 7,8 Go (11,7 avant V4). Seuil : `A_TRAITER` si mémoire moyenne > 60 % ou premier
  OOM — loin.
...
- ✅ **Vu : 2026-09-21 — 10 H APRÈS LE REBOOT, TOUT TIENT.** Noyau **6.8.0-139-generic** (levier 7 : *« à jour »*),
  ...
  16:20, stabilisé depuis »* : des dépendances qui ont attendu leur base au boot, VPS-M38 : historique, pas incident) ;
  swap **256 Ko** (1,1 Go avant), `Committed_AS` **7,8 Go** (11,7), `dockerd` **152 Mo** de RSS (cumul CPU remis à
  zéro) ; `fail2ban` redémarré 16 h 19 min 56 (compteurs à zéro depuis). Le coût V4 (2 min 40) est payé ; le bénéfice se
  lit ici.
...
- 📏 **Vu : 2026-09-21 — 78 invocations/min (72 le 17/09), 29 sondés / 38, 9 SANS SONDE.** 6 conteneurs à 10 s, 19 à
  30 s, 4 à 1 min → **~112 320 / jour**. Le +6/min vient des recréations du 20/09 (démo api/web 13 h 40 sur `latest`,
  `tracky-postgres` 16 h 24) — à lire dans `config.v2.json`, non fait ce passage. `dockerd` *« Health check … timed out
  starting »* : **0** depuis le reboot (×213 à 05 h le 20/09 sous VPS-045) — les sondes obtiennent de nouveau leur CPU.
  `proc/s` de fond **27–28** (`sar -w`, 02 h 10–02 h 20), contre 117 pendant la rafale VPS-047 : *la charge de fond des
  sondes vaut ~un quart de ce qu'une boucle SSH mal écrite a coûté pendant deux heures*.
...
- ✅ **Vu : 2026-09-21 — SEUIL « DEUX PASSAGES » ATTEINT → `SURVEILLANCE` ; LE GARDE-FOU TOURNE, ET LA RÈGLE EST ENFIN LÀ
  ...
  d'uptime (⚠️ la série « cumul » repart de zéro avec le reboot V4 — ne pas y lire une chute : 395,5 h → 0,1 h). Le
  garde-fou : `docker-orphelins.timer` **active / enabled**, dernier passage **36 s** avant la lecture, témoin
  `clients_docker_streaming_vus=0 tues=0` ; **3 « TUÉ » sur 24 h = les 3 du banc du 20/09 13 h 08–13 h 09**
  (`faux-conteneur`, `docker logs -f tracky-lp` sous `timeout`, `faux-chaine`) — **aucune occurrence réelle**. 🔑 `CLAUDE.md`
  porte désormais la règle V34 (section « Toute commande `docker` sur le VPS est BORNÉE », relue ce matin : `timeout 20`,
  `--tail ≤ 2000`, `pgrep -a -x docker` avant tout diagnostic, jamais dans un `bash -c` enchaîné). ⚠️ Le jour 1 sans
  occurrence a quand même vu **7 774 sessions SSH en 2 h** depuis le poste (VPS-047) — *une autre classe d'outil du poste
  qui ne rend pas la main comme prévu ; celle-ci ne touche pas `dockerd`*. **Seuil inchangé** : gravité 1 au premier client
  > 60 s ; `APPLIQUE` après 7 jours de `journalctl -t docker-orphelins` sans « TUÉ » hors banc (jour 1 / 7).
...
- ✅🔴 **Vu : 2026-09-20 — 7ᵉ OCCURRENCE LE 19/09 À 18 H 30 (LE SECOND CŒUR), LES DEUX CLOSES PAR LE PROPRIÉTAIRE
  ...
  2060212), né le **19/09 18 h 30 min 17 UTC**, **4 min après** `deploy.sh perimetre: marketing` (18 h 26 min 48) ; `sar`
  18 h 40 : idle **37,36 → 7,33 %**, `sys` 35 → 53 % — le second cœur, pendant que nº 6 tient le premier depuis le
  16/09 (lu au centre d'alerte à 01 h 17 : 84 h 37 et 6 h 47). Puis l'hôte reprend le CPU par paliers (22 h 10 →
  01 h 10, steal 3 → 89 %) — **VPS-045**. **Vers 05 h 25**, session « Tracky est down » du propriétaire : chaînes de
  parenté relues, **les deux parents puis les deux clients tués** (SIGTERM, pas de `restart docker`), latence
  `/api/health` 5 s → 0,4–1,4 s en 2 min ; `sar` 05 h 20 → 05 h 40 : idle 0,08 → 5,81 → 11,62 %. **Preuve à ce
  passage** : `pgrep -a -x docker` **vide ×4** (10 h 44, 11 h 14, 11 h 29, 11 h 36) ; `dockerd` **0 / 3 / 0,9 %** ;
  cumul **395,4 → 395,5 h** en 45 min (figé) — 319,5 → 395,5 h depuis le 17/09 = +76 h en 79 h 30 (un cœur 62 h, deux
  cœurs 7 h, puis *affamé par le steal* : même la boucle ne tournait plus à plein) ; 0 connexion sur la socket.
  **Sept sur sept depuis le poste** ; le nº 7 ne portait **qu'un** `docker logs` (tuer le client aurait suffi), le nº 6
  en portait trois (parent d'abord). `CLAUDE.md` **ne porte toujours pas la règle** (diff relu ce matin) ; la
  mémoire du poste, si (05 h 24 — elle compte « 7ᵉ et 8ᵉ » : ce sont les nº 6 et nº 7). **Seuil** : `SURVEILLANCE` au
  prochain passage si `pgrep -x docker` est encore vide et `dockerd` < 10 % (1 passage sur 2 acquis) ; **gravité 1
  de nouveau au premier client > 60 s**. ❌ Ne pas attribuer aux deux `kill` le retour du CPU : la part servie est
  passée de 10 à 21 % à 05 h 30 et n'a plus bougé en six heures (VPS-045).
- 🔴 **Vu : 2026-09-17 — 6ᵉ OCCURRENCE, NOMMÉE À LA MINUTE, ET C'EST ENCORE UN `docker logs` SANS
  `timeout` DEPUIS LE POSTE.** Le 16/09 à **12 h 41 min 08 UTC**, session SSH `82.67.153.51:26499`
  (`sshd[3366619]`), `bash -c` de diagnostic *« ── dernières lignes vizyo-auth-api ; docker logs --tail 15
  vizyo-auth-api | cut … ; docker logs --since 10:55Z --until 12:05Z … »* (pid **3366707**, PPID 1,
  **vivant**, `SSH_CONNECTION` lu dans `/proc/<pid>/environ`) → **`docker logs --tail 15 vizyo-auth-api`**
  (pid **3366708**, `futex_wait_queue`, 1 s de CPU) jamais terminé, **52 743 s** ce matin. `sar` 09-16 :
  **85,22 %** d'inactivité à 12 h 40, **42,41 %** sur 12 h 40–12 h 50 (`sys` 4,5 → 30,3) = ≈ 55 s à deux
  cœurs → **la boucle commence vers 12 h 41, AVEC le client — 4 min AVANT la recréation de
  `vizyo-auth-api`** (12 h 45 min 34 ; `dockerd` : ancien conteneur `7e9819…`, 284 h, exit 143,
  *hasBeenManuallyStopped=true*). Puis 37–40 % sur les 75 relevés suivants jusqu'à 03 h 20. `dockerd`
  **98,3 %**, cumul **304,6 → 319,5 h** (+14,9 h = 12 h 41 → 03 h 21 à 10 min près), **1 290 867 `read()`/s
  pour 0 octet**, 59 threads ; 2 connexions établies sur la socket pour 1 processus `docker`. Journée du
  09-16 : **63,33 %** (12,7 h à deux cœurs + 11,3 h à un) ; 09-17 partiel **38,65 %**. Collecte 228 s :
  `dockerd` 43,2 % de la machine, audit 13,4 %, inactivité 7,1 %. ⚠️ **Le conteneur visé n'était PAS dans
  la liste des troués** (VPS-041 : les 6 sont des `-dev-`) ; son journal neuf porte 0 NUL, l'ancien est
  parti avec lui — *le lien NUL ↔ blocage n'est ni confirmé ni infirmé : mesure NON FAITE.* **6 sur 6 = un
  `docker logs` depuis une session du poste qui se ferme avant lui** (outil non nommé, VPS-M01 ; le motif
  *« echo "── …" | cut »* est celui d'un agent). **QUOI FAIRE — V33** : relire `ps -o pid,ppid,lstart,etime,stat,cmd
  -p 3366707,3366708` puis `kill 3366707 && sleep 1 && kill 3366708` — **le parent d'abord** (cas du 08-20 et
  du 13/09) ; preuve : `pgrep -x docker` vide, `dockerd` < 10 %, `sar` à 86 % au relevé suivant. ⚠️ **AVANT
  V4** : un redémarrage efface l'état qui prouve la cause. Puis la **cause**, côté poste (hors périmètre) :
  une ligne dans `CLAUDE.md` — `docker logs` sur le VPS = `timeout 20`, jamais depuis un `bash -c` qui peut
  mourir avant lui. ❌ Ne pas lire « 63,33 % » comme une demi-marge. ❌ Ne pas conclure que V29 ne sert à
  rien — elle ne ferme pas la classe et ne l'a jamais promis. ❌ Ne pas attribuer le raccourcissement des
  passages d'automatisation au cœur rendu (**VPS-M103**). **Seuil** : `SURVEILLANCE` gravité 2 quand
  `pgrep -x docker` est vide **et** `dockerd` < 10 % sur deux passages.
...
- 🔴 **Vu : 2026-09-15 — 46ᵉ HEURE, DEUXIÈME JOURNÉE ENTIÈRE SOUS 50 %, ET LE COÛT D'UN DÉPLOIEMENT
  ...
  pour 0 octet**, cumul **265,3 → 289,4 h** : **+24,1 h de CPU en 24 h 00** — un cœur entier,
  sans une minute de répit. `sysstat`, **lundi 09-14 complet** : `user` **23,6 %**, `sys` **33,2 %**,
  `steal` **4,07 %** (×1,6 : l'hyperviseur reprend ce qu'on brûle), **idle 38,9 %** — deuxième
  journée entière sous le seuil 🟠, après 47,7 % samedi ; `sar` heure par heure : **38 à 41 % à
  chaque relevé**, deux fosses à **7,95 %** (08 h 20, le déploiement) et **8,37 %** (11 h 50, le
  build retenu). Le 09-15 partiel (nuit) est à 39,5 %. ⚠️ **Si le 09-15 finit sous 50 %, le
  collecteur rendra 🔴 le 16/09** (« trois jours de suite ») — sans que rien d'autre ait changé.
  🆕 **Le coût, mesuré pour la première fois** : `journal.jsonl` porte `deploy.sh` du **09-14
  08 h 20 min 38** (`9afdf52a`) en **1 916 s = 31 min 56**, contre 303 à 585 s les sept passages du
  09-13 — **×3,3 à ×6,3**. ⚠️ *Les 303–585 s étaient DÉJÀ à un cœur* : ce n'est pas « le coût de
  VPS-016 », c'est la variance d'un build à un cœur sur une journée qui portait aussi 7 passages
  d'automatisation de plus de 45 min. **Aucune durée à deux cœurs n'existe** : V28 avant le
  prochain déploiement, ou la série ne comparera jamais rien. Collecte à **7,5 %** d'inactivité,
  `dockerd` **45,4 %** de la machine, production **78–201 ms**. La stanza `logrotate` a re-percé à
  00 h 00 min 04 (**13,2 s de CPU**, VPS-041) ; `texto-relay` **4ᵉ jour à 0 octet**.
  **Remédiation inchangée (V28)** : PID vérifiés à 02 h 28 min 55, parent d'abord.

  | Occurrence | Début | Fin | Durée | Client | Parent |
  |---|---|---|---|---|---|
  | 5 — **2026-09-13** | **04 h 34** | *en cours à 02 h 28 le 09-15* | **> 45 h 55** | `docker logs --tail 60 texto-relay` | **vivant**, PPID 1 |

- 🔴 **Vu : 2026-09-14 — 22 HEURES PLUS TARD, RIEN N'A CHANGÉ, SAUF LA MACHINE : PREMIÈRE JOURNÉE
  ENTIÈRE SOUS 50 % D'INACTIVITÉ.** À 02 h 27 min 35 UTC, `159533` (PPID 1, `do_wait`) et `159541`
  (`futex_wait_queue`) sont **vivants, 78 814 s** ; `dockerd` **101,0 %**, **1 367 067 `read()`/s
  pour 0 octet**, cumul **249,0 → 265,3 h** : **+16,3 h de CPU en 16 h 14 écoulées** — un cœur
  entier, en continu, et l'âge du client a avancé de **58 439 s** pour 58 439 s d'intervalle.
  `sysstat`, **journée complète du 09-13** : `user` **21,0 %**, `sys` **28,1 %**, **idle 47,7 %**
  contre 85–89 % les sept jours d'avant — **le seuil 🟠 « MARGE ENTAMÉE » du collecteur (< 50 % sur
  une journée entière) est franchi pour la première fois** ; le 09-14 partiel (nuit, aucun
  déploiement) est à **40,2 %** : c'est le régime de fond avec un cœur confisqué. Collecte à
  **7,6 %** d'inactivité, `dockerd` **45,4 %** de la machine, production **82–169 ms** (45–76 hier
  dans le même état — la latence de la collecte, pas de l'application).
  🆕 **Ce que la machine a fait pendant ce temps** : `/opt/tracky-deploiements/journal.jsonl`
  (T33, posé le 09-13 12 h 37) porte **sept déploiements `deploy.sh`** le 09-13 — 12 h 37 (367 s),
  13 h 03 (303), 13 h 33 (397), 17 h 37 (**507**), 18 h 03 (**585**), 20 h 02 (324), 20 h 25
  (392) — **48 min de build cumulées sur un seul cœur**. *Aucune ligne de ce journal n'existe
  avant VPS-016 : la durée « normale » d'un déploiement n'est pas mesurable tant que V28 n'est pas
  faite — un argument pour la faire **avant** le prochain, pas après.* ⚠️ Seuil suivant du
  collecteur : 🔴 sous 25 % d'inactivité, ou 🟠 **trois jours de suite** — le 09-14 sera le 2ᵉ.
  🆕 La stanza `logrotate` a **re-percé à 00 h 00 min 10** : `texto-relay` est à **0 octet pour le
  3ᵉ jour** (VPS-041). **Remédiation inchangée (V28)** : PID vérifiés à 02 h 27, parent d'abord.

  | Occurrence | Début | Fin | Durée | Client | Parent |
  |---|---|---|---|---|---|
  | 5 — **2026-09-13** | **04 h 34** | *en cours à 02 h 27 le 09-14* | **> 21 h 53** | `docker logs --tail 60 texto-relay` | **vivant**, PPID 1 |
...
- ✅ **Vu : 2026-09-21 — 24ᵉ PRÉDICTION JUSTE, ET LA PÉRIODE CHANGE : 4 JOURS AU LIEU DE 2 (V32 (a)).** `alpine:latest`
  ...
  00 h 40 le supprimera »* sur une image que ce ménage venait d'épargner — **VPS-M109**, corrigé ce passage (seuil lu dans
  le cron). **Prédiction réécrite** : supprimé au ménage du **23/09 00 h 40** (93 h), re-tiré par la sauvegarde Verify du
  23/09 03 h 31 ; puis tous les 4 jours. La sauvegarde de cette nuit (03 h 30) **ne tirera pas**. V16 (épingler par
  empreinte) reste le seul geste qui ferme la fiche.
...
- 🔴 **Vu : 2026-09-21 — 7 946 SESSIONS LE 20/09 (RECORD ×1,9), 7 774 EN DEUX HEURES : CE N'EST PLUS CE CONSTAT, C'EST
  VPS-047.** Par jour : 468 (bord) · 607 · 557 · 935 · 132 · 121 · **7 946** · 27 (bord) ; total 7 j **10 793** (3 859 hier).
  Ventilé ce passage (VPS-M110, angle mort n° 4 **traité au 10ᵉ report**) : **7 941 depuis le poste** (`82.67.153.51`, clé
  `vizyo-vps-hostinger`), 5 depuis `179.198.198.199` (Conductor ×3, Dispocar ×2) ; par heure : ≤ 29 partout, sauf **14 h
  2 563 · 15 h 3 894 · 16 h 1 317** — une boucle `Monitor` du poste dont le `sleep` ne dormait pas (**VPS-047**, cause
  identifiée dans les transcriptions). *Le régime de fond, lui, n'a pas bougé* : hors rafale, le 20/09 porte **172
  sessions** (journée du dimanche + reboot) et le 21/09 partiel 27 — la pause des agents locaux (T75) tient toujours
  ~120 / j. Ce constat garde sa gravité 3 pour le **bruit de fond** ; la **rafale** a sa fiche, gravité 2.
- 🔑 **Vu : 2026-09-20 — LA CHUTE DE 935 À 132 SESSIONS/JOUR COÏNCIDE AVEC LA PAUSE DES AGENTS LOCAUX DU POSTE : PREMIÈRE
  INDICATION DE *QUI*.** Par jour : 891 (bord) · 506 · 607 · 557 · **935 (17/09) · 132 (18/09) · 121 · 110** ; total 7 j
  **3 859** ; `vizyo-vps-hostinger` **3 820** connexions ; `82.67.153.51` 3 676. Le centre d'alerte date la pause
  `echecs-consecutifs` des agents locaux Tracky au **17/09 08 h 50** (TRK-069 / T75, `leveeA` NULL) — et le 18/09 est le
  premier jour à ~130. *Par déduction, pas par mesure* : ~**600 à 800 sessions par jour** seraient les agents locaux
  (`RecitTrajet`, `QualiteGPS`, `LimitesVitesse`, `CourrierIA`, rattrapages toutes les 2 h) ; le reste (~120) = les
  audits, `deploy.sh`, et les sessions humaines. **À confirmer à la reprise des agents** : si le compte remonte à
  ~600 le lendemain de « Reprendre maintenant », la ventilation est acquise. *La question ouverte du 17/09 — « qui
  parle à cette machine ? » — a sa première réponse chiffrée, obtenue par une absence.*
- 🔑 **Vu : 2026-09-14 — LES TROIS JOURS DE QUOTA DONNENT LE PLANCHER QUE PERSONNE N'AVAIT MESURÉ.**
  **4 492** sessions / 7 j ; par jour : 641 · 459 · **159 · 177 · 111** · **1 055** · 38 (partiel).
  Les 10, 11 et 12/09, aucune des trois routines du poste n'a tourné (plafond de la CLI, VPS-M73) :
  **~150 sessions/jour sans routines** — c'est le fond (copie hors-site, outillage, main). Le
  09-13 : 7 déploiements `deploy.sh` + 3 routines rattrapées = **1 055**. *Une baisse de ce
  compteur mesure l'absence de l'observateur avant tout correctif (rappel du 13/09) ; ce passage
  en tire au moins la valeur du plancher.* 🆕 Et les sessions **établies** (durée, pas ouverture)
  sont désormais imprimées en section 6 — angle mort n° 2 du 13/09, traité.
...
- ✅ **Vu : 2026-09-23 — LE TEST DATÉ EST PASSÉ : 7 → 1, ET LES 7 SONT NOMMÉS.** `history.log` 22/09 06 h 53 min 14 →
  06 h 53 min 44 (`unattended-upgrade`) : `libglib2.0-bin`, `gir1.2-glib-2.0`, `libglib2.0-data`, `libglib2.0-0t64`,
  `libexpat1`, `libxml2`, `rsyslog` — **exactement 7**. Compte sur cache frais (01 h 37 ce matin) : **1**, découvert depuis ;
  prochaine installation 06 h 40. *La mesure est quotidienne ET le canal installe : les deux moitiés de la fiche tiennent.*
  Effets de bord lus : `fail2ban` redémarré 06 h 53 min 31 (compteurs remis à zéro) ; 2 services sur une bibliothèque remplacée.
...
- ✅ **Vu : 2026-09-21 — V14, PREMIÈRE PREUVE SUR DEUX : « cache de 0 h — MESURE VALIDE » À 02 H 25.** `apt-daily.timer`
  ...
  01 h 33 min 48. **70 paquets en retard, 0 estampillé sécurité** — sur cache frais, pour la première fois **par
  construction** et non par chance (5 mesures valides sur 21 avant). `apt-daily-upgrade` prévu **06 h 36 min 37** (la
  veille : 06 h 20 min 56, a installé — journal VPS-M108) ; `reboot-required` : non ; 0 service sur bibliothèque remplacée
  (V4 a tout rechargé). *Seuil : `APPLIQUE` à la seconde mesure valide de suite, demain 02 h 2x.*
...
- 🟠 **Vu : 2026-09-17 — NON MESURABLE (4 valides / 20), MAIS LE CANAL EST PROUVÉ : LES 3 CORRECTIFS D'HIER
  ...
  06 h 52 min 24 *« Packages that will be upgraded: libpolkit-agent-1-0 libpolkit-gobject-1-0 polkitd »*,
  06 h 52 min 39 *« All upgrades installed »* ; `dpkg.log` 06 h 52 min 26 ×3 (124-2ubuntu1.24.04.3 → .4) ;
  `update-notifier` **64 → 61** ; le cache périmé rend 73 → 70 dont **3 → 0**. *Ce qui restait en doute était
  la mesure, pas l'installateur.* V14 reste le seul geste.
...
- **Vu : 2026-09-15 — 15ᵉ échec sur 18, CONFORME À LA LOI DU 14/09.** Cache rafraîchi le **09-14
  ...
  rafraîchira (1ʳᵉ du 09-15) pour rien. Indicatif seulement : 70 en retard, 0 sécurité,
  `update-notifier` de la même minute. V14 inchangée.
- ✅ **Vu : 2026-09-14 — NON MESURABLE (14ᵉ échec sur 17), ET LA MÉCANIQUE EST ENFIN ÉCRITE : ELLE
  PRÉDIT SON PROPRE TAUX D'ÉCHEC.** Cache daté du **09-13 03 h 02 min 06**, collecte à 02 h 22 :
  **23 h**. Le même cache qu'hier — *une mesure publiée deux fois, VPS-M74 encore*.
  **Ce que le code dit** (`systemctl cat apt-daily.timer`, `journalctl -u apt-daily.service`,
  `/usr/lib/apt/apt.systemd.daily`) :
  1. `apt-daily.timer` : **`OnCalendar=*-*-* 6,18:00`**, `RandomizedDelaySec=12h`, `Persistent=true`
     — **deux sonneries par jour**, chacune décalée d'un aléa uniforme sur 12 h ;
  2. `check_stamp()` compare **minuit du jour du tampon à minuit d'aujourd'hui** (le commentaire le
     dit : *« compare midnight today to midnight the day the stamp was updated »*) : **la première
     sonnerie de chaque jour UTC rafraîchit, la seconde ne fait rien** ;
  3. le journal le montre à chaque jour : 09-13 **03 h 01 min 51 → 19,9 s de CPU** (rafraîchit ;
     `update-success-stamp` 03 h 02 min 06), puis 15 h 12 et 19 h 56 **→ 1 s** (rien) ; 09-12 07 h 56
     (8,7 s) ; 09-11 13 h 40 (27,7 s) ; 09-10 02 h 23 (13,2 s) ; 09-09 01 h 29 (19,3 s) ; 09-08 05 h 28
     (20,3 s). `LastTriggerUSec` (19 h 56) ne montre que la **dernière** sonnerie — c'est pourquoi
     le rafraîchissement de 03 h 02 semblait venir de nulle part.
  **Conséquence arithmétique** : le cache est frais (< 6 h) à 02 h 20 **si et seulement si** la
  sonnerie de 18 h de la veille tombe entre **00 h 00 et 02 h 20** — **2 h 20 / 12 h ≈ 19 %** des
  jours. **Mesuré depuis le 08-20 : 3 succès sur 17 = 18 %.** *La série d'échecs n'était pas de la
  malchance, c'était une loi.*
  🔑 **Ce que ça change pour V14** : le geste du 07/09 (`OnCalendar=*-*-* 01:30:00` +
  `RandomizedDelaySec=15m`) est **compatible avec `check_stamp`** — à 01 h 30–01 h 45 le tampon
  porte la date de la veille, donc l'unique sonnerie rafraîchit **chaque jour, 50 min avant
  l'audit**. ⚠️ Ce qui **ne** marcherait **pas** : garder deux sonneries et réduire l'aléa — la
  seconde resterait un no-op et la première tomberait entre 06 h et 18 h, jamais avant l'audit.
  ✅ Effet de bord vérifié : `apt-daily-upgrade` (06 h 00 + aléa 60 min) lit le même tampon et
  continuera d'installer après le rafraîchissement de 01 h 30, comme aujourd'hui après celui de
  03 h 02. ⚠️ `systemctl edit` ouvre un éditeur : geste humain, hors audit.
...
- 🟢 **Vu : 2026-09-23 — À L'HEURE, SUR SECTEUR, ET LA TÂCHE DE RÉVEIL A SONNÉ POUR LA PREMIÈRE FOIS — SUR UN POSTE QUI NE
  ...
  `Kernel-Power 107` à 04 h 15 : le poste était éveillé depuis **23 h 22** (`Kernel-Power 507`). *La tâche a prouvé qu'elle se
  déclenche sur secteur, pas qu'elle réveille.* `powercfg /a` : veille prolongée **toujours indisponible** (fichier `reduced`) —
  la moitié admin de V6 n'est pas faite. Audit parti **02 h 29 UTC (+7 min)**. Côté VPS : JSON du **21/09 04 h 30** (46 h),
  *« 🔴 1 paire produite DEPUIS la dernière copie réussie »* (celle du 22/09) ; la copie de 04 h 30 UTC tombe **après** ce
  passage — **`pairesCopiees = 2` attendu** (22/09 + 23/09), à lire demain. Gravité inchangée (seuil 1 : 3 paires / 7 j).
...
- ✅ **Vu : 2026-09-21 — CET AUDIT EST PARTI À L'HEURE (02 h 23, +1 min), SEUL, ET LA COPIE DU 20/09 A TENU (5ᵉ NUIT).**
  ...
  **après** ce passage : à lire demain. Le poste était éveillé à 02 h 22 (`✅ heure de départ : 02:23 UTC`) — mais
  `WakeToRun` n'a pas été posé (V6) : *c'est encore la chance*. La « seconde session » vue pendant la collecte est **mon
  propre pré-contrôle V34** (`pgrep -x docker`, 02 h 23 min 09, 14 s avant `T_DEBUT`) — identifiée, pas une collision
  (VPS-M57 : même cas que le 22/08).
- ✅ **Vu : 2026-09-20 — QUATRE NUITS DE COPIE RÉUSSIE : LE SEUIL DE GRAVITÉ 1 N'A PAS ÉTÉ FRANCHI, ET RIEN N'A ÉTÉ
  CORRIGÉ.** `copie.log` (poste) : **17/09 06 h 30 min 13 OK, 2 paires** (16 + 17 — le rendez-vous de 04 h 30 UTC a
  eu lieu), 18/09 OK 1, 19/09 OK 1, **20/09 06 h 30 min 36 OK 1** (27 s, rotation de 3 fichiers) ; JSON du VPS
  **2026-09-20T04:30:31Z**, `pairesCopiees 1`, `pairesLocales 15`, `plusRecente verify-db_20260920-033038` ;
  **0 manifeste plus récent que le JSON**. `WakeToRun` **toujours `False`**, `StartWhenAvailable True`, `Next 21/09
  06:30`. *Le symptôme s'est refermé parce que le poste était éveillé quatre matins de suite* (VPS-015, même motif :
  il se rouvrira tout seul). ⚠️ Le même poste a pourtant **manqué ou retardé trois audits VPS** sur la même période
  (VPS-M73) — les deux devoirs ne dorment pas toujours ensemble. Geste inchangé : `WakeToRun`, 30 s ; V6 due.
- 🔴 **Vu : 2026-09-17 — LE POSTE A DORMI DE 23 H 17 À 03 H 15 UTC : 2 PAIRES PRODUITES DEPUIS LA
  DERNIÈRE COPIE RÉUSSIE N'EXISTENT QU'ICI, ET LE COLLECTEUR ÉCRIVAIT « À JOUR ».** Journal Windows :
  veille **23 h 16 min 59 Z** (événement 42), réveil **03 h 15 min 38 Z** (événement 1, source *« Unknown »*).
  Conséquences : cet audit à **03 h 18** (+56 min), **les deux audits lancés à 03 h 16 min 25** (221 ms
  d'écart — VPS-M57, 3ᵉ collision), et la copie du 16/09 04 h 30 **n'a pas eu lieu** (`WakeToRun` toujours
  `False`, `NextRunTime 17/09 06:30`). JSON du VPS : **15/09 04 h 30 min 10** ; paires produites depuis :
  `verify_20260916-033130` (5,5 Mo) et `verify_20260917-033056` (03 h 31, pendant ce passage). ⚠️ La ligne
  disait **`OK 46 h à jour`** — le seuil de 48 h ne parle qu'au **2ᵉ** rendez-vous manqué, et « à jour » se
  lit vert ; *hier j'avais prédit « PÉRIMÉE (46 h) », faux sur le seuil, et ce seuil était le défaut.*
  **Corrigé** : trois tiers (≤ 30 h · 30–48 h 🟠 RENDEZ-VOUS MANQUÉ · > 48 h PÉRIMÉE) **et** le compte des
  manifestes plus récents que le JSON (*« 🔴 2 paire(s) produite(s) DEPUIS la dernière copie réussie — elles
  n'existent qu'ICI »*). **Le seuil de gravité 1 (3ᵉ paire sans copie sur 7 j : 09-14, 09-16, …) se joue ce
  matin à 04 h 30 UTC**, selon que le poste reste éveillé. Geste inchangé : `WakeToRun`, 30 s ; puis V6.
...
- 🔴 **Vu : 2026-09-15 — LE SEUL COPIEUR A ÉCHOUÉ (09-14 04 h 30, `pairesCopiees 0`), ET IL N'Y A
  ...
  paire du 09-14 n'existe que sur le VPS, et **aucun second dépositaire ne l'a prise**. Le
  collecteur affichait pourtant « à jour » (VPS-M100, corrigé). ⚠️ **V6 ne corrige pas VPS-043** :
  un second copieur bâti sur la même logique s'arrêterait au même manifeste — les deux tâches sont
  distinctes et **V31 passe avant**. Le rapport du 14/09 avait publié « 21 h, inchangé » sur un
  fichier qui disait ÉCHEC.
...
- ✅ **Vu : 2026-09-21 — DIMANCHE 09-20 COMPLET = 34, HUITIÈME JOURNÉE ≥ 32, ET LE REGISTRE NE BOUGE PAS.** 3 comptes =
  **34 / 34 / 34** (positions, wire_logs, décisions — contrôle croisé VPS-M89 ✅) ; positions **14 222** / 24 h = ×0,62 du
  ...
  (121 hier), ×0,95 ; `FRAÎCHEUR : aucun émetteur muet depuis plus de 6 h` sur wire_logs et décisions. Registre : **10
  silencieux > 7 j sur 44**, cumuls > 1 j / > 3 j / > 7 j = **10 / 10 / 10** (inchangés depuis le 13/09) — la sentinelle V24
  n'a rien de neuf à dire. Positions : *« 8 émetteurs silencieux, dernier arrêt 09-03 »* = les mêmes 8 de la cohorte du
  08-31 encore dans la fenêtre de 61 j (plancher qui décroît, VPS-M76) ; le registre en compte 10 = les 2 sortis de la
  fenêtre. Seuil tenu, 8ᵉ jour.
- ✅ **Vu : 2026-09-17 — MERCREDI 09-16 COMPLET = 33, SIXIÈME JOURNÉE ≥ 32.** Série calendaire : 30 · 30 ·
  33 · 32 · 32 · 33 · 33 · **33** · (09-17 partiel 32). Les trois compteurs **33 / 33 / 33** (3ᵉ égalité).
  Registre **11 / 11 / 11**. **`…6714`** : dernière position **09-16 15 h 28 min 34** (il a donc mesuré
  hier après-midi), **1 612 trames / 24 h**, `lastSeenAt` 03 h 30 — 4ᵉ épisode « trames sans position ».
  10 silencieux > 6 h sur `positions`, 0 sur `wire_logs` : garés. Seuils inchangés.
...
- ✅ **Vu : 2026-09-15 — LE LUNDI 09-14 COMPLET REND 33 : QUATRIÈME JOURNÉE ≥ 32, ET LES TROIS
  ...
  **33** ; sur 24 h, `positions` **33** / `wire_logs` **33** / `position_sampling_decisions` **33** —
  première égalité parfaite depuis le 09-11. L'écart de 1 des jours précédents était `…6714`, et
  il a **produit des positions le 09-14 jusqu'à 14 h 23 min 06** (première depuis le 09-11 16 h 19),
  puis de nouveau **des trames sans position** (10 747 trames en 4 j, dernière 02 h 29 min 55 ;
  `lastSeenAt` à jour) : *il s'annonce, et mesure par intermittence* — à porter **à part** (V1).
  Registre **11 / 11 / 11**, tous > 7 j (9 à 102 jours : `…7662` 9 j, `…6649` 12 j, `…6755`
  `…6102` `…7027` 15 j, `…5856` 17 j, `…6177` 25 j, `…6730` 27 j, `…7480` 32 j, `…6292` 76 j,
  `…0776` 102 j). Part `MOVING` lundi **18,0 %** (dimanche 10,6, samedi 17,9) : le cycle tient.
- ✅ **Vu : 2026-09-14 — LE SAMEDI 09-13 COMPLET REND 32 : TROISIÈME JOURNÉE ≥ 32, RIEN DE NEUF.**
  `positions` par jour calendaire : 30 · 30 · 30 · **33 · 32 · 32** (09-14 partiel : 32). Les trois
  compteurs **32 / 33 / 32** ; l'écart de 1 reste **`…6714`** : **3 305 trames le 09-13, 328 ce
  matin, 0 position** depuis le 09-11 16 h 19 (3ᵉ jour — c'est lui la ligne `🔴 SILENCE : 1
  emetteur` de la table des décisions, dernier arrêt 09-11 16 h 19). **Registre 11 / 11 / 11** :
  les 11 silencieux sont tous **> 7 j** désormais (bande `0/0/0/11` contre `0/0/1/10` hier) — le
  cumul `> 3 j` **n'a pas bougé** (11 → 11) : *c'est du temps qui passe, pas un boîtier de plus
  (VPS-M90)*. Par date de dernière vue : 06-05, 07-01, 08-14, 08-19, 08-21, 08-29 (les 6 déposés),
  **08-31 × 3** (cohorte), 09-03, 09-06. ⚠️ Mesure à **02 h 22**, à l'heure : les fenêtres « 24 h »
  redeviennent comparables à celles d'avant le 10/09. Seuils inchangés (`SURVEILLANCE` à 38 ;
  gravité 1 sous 30 ou si un revenu se tait > 3 j).
...
- 🔴 **Vu : 2026-09-20 — L'IMPORT HEBDOMADAIRE A ÉCHOUÉ (schéma de la démo en retard de trois déploiements) → fiche
  dédiée VPS-046.** Base démo **581 Mo** (541 le 17/09, +40 sans import réussi : le rejeu applicatif écrit) ;
  `tracky-demo-refresh-demande` sonde toutes les 15 min (11 h 15, 11 h 30 : 8–11 s chacune, rien à faire) ; 4
  conteneurs `tracky-demo-*` dans les 18 hors production que V35 (2) propose d'arrêter.
- 🔁 **Vu : 2026-09-15 — LA LECTURE D'HIER ÉTAIT FAUSSE DANS LES DEUX SENS : LA DÉMO N'A JAMAIS
  ANALYSÉ, ET RIEN NE S'EST ARRÊTÉ.** `trip_automation_settings` sur `tracky_demo` :
  **`enabled = false`** (réglé le **09-07 14 h 45**), **`lastRunAt` NUL**. Les 11 753 `trip_analyses`
  (max `computedAt` 09-12 22 h 48) sont **importées** de la production avec leurs horodatages — les
  « 121 arrivées » du 14/09 étaient des lignes copiées dont le `computedAt` de *production* tombait
  dans la fenêtre (🆕 **VPS-M101**, corrigé : le collecteur imprime désormais l'interrupteur). La
  question produit de V27 (*« voulu ou effet de l'import ? »*) est **close sur le fait** : voulu, par
  réglage. Base **518 → 525 Mo** ; `trips` 180 / 24 h (×2,37 veille, ×1,37 J-7 : calendrier, pas
  débit) ; `positions` 16 287 / 24 h, 37 émetteurs de rejeu. ➜ **V27 n'a plus qu'une décision** :
  ne pas sauvegarder (525 Mo/j, reconstruite par l'import).
- 🟠 **Vu : 2026-09-14 — DEPUIS L'IMPORT DE DIMANCHE, LA DÉMO NE PRODUIT PLUS AUCUNE ANALYSE DE
  TRAJET.** Base **514 → 518 Mo**. Section 5 sur `tracky_demo` : `trip_analyses` **0 arrivée sur
  24 h contre 121 les 24 h précédentes** (×0,00 contre la veille **et** contre J-7 : *« l'excursion
  n'est PAS un effet de calendrier »*), `max(computedAt)` = **09-12** — l'import de 04 h 04 a
  **recopié** les 11 753 analyses de la production (table `recalé 09-13 04:04`) et rien n'en a
  écrit depuis ; `trips`, lui, continue (76 / 24 h, 17 émetteurs). *La production, elle, analyse :
  191 · 171 · 181 · 150 · 68 (dimanche) · 1.* ⚠️ **Question produit, non tranchée** : la démo
  n'a-t-elle pas d'automatisation (voulu, pas d'IA sur la démo), ou l'import l'a-t-il éteinte ?
  L'audit ne lit pas les réglages du conteneur `tracky-demo-api` — angle mort n° 4 du 14/09.
  Périmètre du parc inchangé (4 conteneurs, 4/4 avec limite mémoire).
...
- ✅ **Vu : 2026-09-21 — PREMIÈRE NUIT À UN SEUL ROTATEUR : 0,6 s DE CPU, AUCUN NOUVEAU TROU, `tracky-postgres` SORTI.**
  ...
  Docker (aucune stanza logrotate hôte ne vise ces fichiers) »*. Troués **6** — `maalem-dev-api` 48 % (71 % hier),
  `maestroo-dev-api` 21 % (45), `maestroo-dev-web` 84 % (95), `maalem-dev-web` 91 % (96), `maalem-dev-admin` 92 % (97),
  `maestroo-dev-lp` 83 % (94) — **tous des `-dev-`, tous en BAISSE** (le trou est fixe, le fichier grossit : c'est le
  signe qu'on n'a plus percé) ; **`tracky-postgres` sorti** (recréé 16 h 24, journal neuf, `max-file 14` lu dans
  `config.v2.json` par la section 4 : `memlimit=1610612736`, `healthy`). Total journaux **49,9 Mo**,
  `/var/lib/docker/containers` **245 Mo** (318 hier : les 15 fichiers de l'ancien postgres sont partis avec lui — copiés
  dans `/root/journaux-tracky-postgres-avant-v29-2026-09-20/`, 75 Mo). **V29 bis** inchangée : les 316 `*.log.N` des autres
  conteneurs (434 Mo) attendent le 04/10. *1 passage sur 2 vers `APPLIQUE`.*
...
- **Vu : 2026-09-20 — 8ᵉ NUIT : `tracky-postgres` EST REVENU DANS LES TROUÉS (7), ET LA ROTATION A PRIS 3 MIN 40
  ...
  `maestroo-dev-web` 95 %, **`tracky-postgres` 45 % sur 8,1 Mo** (revenu : recréé le 15/09, roté depuis),
  `maalem-dev-web` 96 %, `maalem-dev-admin` 97 %, `maestroo-dev-lp` 94 % ; **9** courants à 0 octet (11 h 2x — ne se
  compare pas aux 13 de 03 h). Le `docker logs` du 19/09 visait `foodsqan-traefik` (non troué, `health=aucune`) :
  *comme le 16/09, la 7ᵉ occurrence ne passe pas par un journal troué — V29 sert à `docker logs` exact, pas à fermer
  la classe.* Trois nuits sans passage (18, 19) : la série de CPU a un trou.
- **Vu : 2026-09-17 — 5ᵉ NUIT : 21,9 s DE CPU, LES MÊMES 6 TROUÉS, ET LA 6ᵉ OCCURRENCE DE VPS-016 NE
  VISAIT PAS UN TROUÉ.** `logrotate.service` **00 h 00 min 00 → 00 h 01 min 17**, **21,92 s** de CPU (1,63 s
  hier, 13,2 avant-hier : `copytruncate` copie avant de tronquer, et les fichiers de production recréés le
  15/09 ont regrossi). Troués **6** : `maalem-dev-api` 80 %, `maestroo-dev-api` 93 %, `maestroo-dev-web`
  96 %, `maalem-dev-web` 98 %, `maalem-dev-admin` 98 %, `maestroo-dev-lp` 94 % ; **13** courants à
  0 octet (03 h 2x). ⚠️ Le `docker logs` bloqué du 16/09 12 h 41 visait **`vizyo-auth-api`** — pas dans
  cette liste, journal neuf à 0 NUL, ancien fichier parti avec le conteneur recréé à 12 h 45 : *V29 reste
  utile (`docker logs` exact), elle ne ferme pas la classe de VPS-016, et ce passage le mesure.*
...
- 🔴 **Vu : 2026-09-15 — TROISIÈME NUIT : 13,2 s DE CPU POUR PERCER, ET LE COMPTE NE MONTE PLUS,
  ...
  Fichiers courants **troués : 5** — `maalem-dev-web` 98 %, `maalem-dev-admin` 98 %,
  `maestroo-dev-web` 95 %, `maalem-dev-api` 92 %, **`maestroo-dev-lp` 84 % (entre)** ;
  `tracky-postgres` et `maestroo-dev-api` **sortent** — rotés par Docker à 10 Mo, le trou est
  parti avec le fichier, et il reviendra à la prochaine troncature. Fichiers courants **à 0 octet :
  17** (18 hier) — `vizyo-vault` entre, `vizyo-auth-web` et `tracky-demo-web` sortent ;
  **`texto-relay` 4ᵉ jour à 0 octet**. *Chaque conteneur fait le tour : troué → roté → troué.*
  Geste inchangé (chemin de retrait daté du jour), **avant minuit** ; preuve attendue inchangée.
- 🔴 **Vu : 2026-09-14 — LA STANZA A RE-PERCÉ CETTE NUIT, ET LE COMPTE MONTE.** `logrotate.service`
  **00 h 00 min 10 → 00 h 00 min 29** (1,98 s de CPU). Fichiers courants **troués : 6** (5 hier) —
  `maalem-dev-web` **98 %**, `maalem-dev-admin` **98 %**, `maestroo-dev-api` **92 %**,
  `maestroo-dev-web` **88 %** (*s'ajoute*), `tracky-postgres` **79 %**, `maalem-dev-api` **73 %** ;
  fichiers courants **à 0 octet : 18** (13 hier) — `capcom6-mysql`, `capcom6-worker`,
  `dg-epaviste-website`, `dronely-devis`, `dronely-presentation`, `foodsqan-traefik`,
  `maalem-dev-minio`, `maalem-dev-postgres`, `maestroo-dev-postgres`, **`texto-relay` (3ᵉ jour)**,
  `tracky-demo-redis`, `tracky-demo-web`, `tracky-redis`, `vizyo-auth-web`, `vizyo-manager-api`,
  `vizyo-manager-dashboard`, `vizyo-manager-postgres`, `vizyo-verify-postgres`. *Un conteneur calme
  tronqué à minuit reste vide toute la journée ; son `docker logs` rend du vide, et du vide se lit
  « rien à signaler ».* ⚠️ **La seule fenêtre où V29 ne coûte rien est avant minuit** : le 15/09 à
  00 h 00, ça recommence. Geste inchangé.
...
- 🟠 **Vu : 2026-09-23 — DISPOCAR `prd` 1 172 O, 11ᵉ NUIT ; ET CONDUCTOR `prd` A PRIS ×8 EN UNE NUIT.** Sous-lignes
  M111 : Dispocar `prd-dispocar-20260923-033353` **1 172 o** 🟠, `dev` 49 553 o ✅ ; Conductor `prd-vizyo_conductor-20260923-030853`
  **288 710 o** (36 142 hier), `dev` 40 848 o. Le saut de Conductor se lit comme une **activité de l'expéditeur** (mise en
  service réelle ?), pas comme un défaut d'ici — à confirmer par la même question que V30 (f). Dépôts 01 h 3x / 01 h 0x UTC,
  14 connexions chacun sur 7 j.
...
- 🔵 **Vu : 2026-09-21 — LA SESSION `vaultbk` DE 253 H EST PARTIE AVEC LE REBOOT, PAS AVEC UNE DÉCISION ; LES DÉPÔTS
  ...
  `vizyo-conductor-distant` **28 copies**, dernière **01 h 03** (`dev-vizyo_conductor-20260921-030307`) ;
  `dispocar-distant` **16 copies**, dernière **01 h 35** (`dev-dispocar-20260921-033553`) — ⚠️ la taille de `prd` n'est
  **pas lue** ce passage (le bloc « âge par dossier » ne montre que le plus récent) : **8ᵉ nuit à 1 172 o NON MESURÉE,
  pas démentie**. Comptes **4 = 4** ; dossiers **13 = 12 + 1** : `instantanes-chantier-20260915` **NEUF** — c'est V17
  (20/09 13 h 12, 4 archives déplacées, `700`), **reconnu**, pas un dépôt d'un tiers ; le bloc VPS-M92 l'a marqué 🟠 « à
  reconnaître (VPS-042) » comme prévu, et c'est ici qu'on le reconnaît. Clés : `depot-sauvegarde@dispocar` 19 connexions,
  `@conductor` 15, `sauvegarde-coffre@conductor` 7 — toutes `restrict` + `command=`. V30 (c) *« fermer ou assumer la
  session »* est **sans objet** désormais ; (a), (b), (d), (e), (f) inchangées.
- 🔵 **Vu : 2026-09-20 — `prd` DISPOCAR À 1 172 o POUR LA 7ᵉ NUIT (14 → 20/09), `dev` VIVANTE (48 813 o).**
  `dispocar-distant` 14 dumps ; `conductor` `prd` 29 754 o à 01 h 04 ; session `vaultbk` depuis `179.198.198.199`
  **253 h**, aucune commande portée ; comptes **4 = 4**, dossiers **12 = 12** ; clés `depot-sauvegarde@dispocar` 17
  connexions, `@conductor` 15, `sauvegarde-coffre@conductor` 7 — toutes `restrict` + `command=`. V30 inchangée : la
  question `prd` se tranche côté Dispocar.
- 🔵 **Vu : 2026-09-17 — DISPOCAR `dev` N'EST PLUS VIDE, `prd` L'EST TOUJOURS : LA QUESTION DE V30 SE
  PRÉCISE.** `dev-dispocar-20260917-033853.sql.gz.age` = **44 064 o** (1 172 o les trois nuits d'avant) ;
  `prd-dispocar-20260917-033434` = **1 172 o, 4ᵉ nuit**. *Le script côté Dispocar vise donc probablement
  une base `prd` vide ou inexistante — se vérifie côté Dispocar, pas ici.* Conductor : **20** fichiers, `prd`
  29,7 Ko déposé à **03 h 06** (l'heure a glissé de 01 h 0x — à surveiller, pas à interpréter). Session
  `vaultbk` **172 h**, aucune commande. Comptes **4 = 4**, dossiers **12 = 12** : première vraie différence
  des deux clés posées hier (VPS-M92).
...
- 🆕 **Vu : 2026-09-15 — UN TROISIÈME DÉPOSANT EST ARRIVÉ DIMANCHE À 05 H 32, ET CETTE FOIS LE
  ...
  **`dispocarbk`** (uid 1001, `/bin/sh`, commentaire *« depot des dumps DISPOCAR
  (recevoir-dump-dispocar seulement) »*), clé `depot-sauvegarde@dispocar` avec
  **`command="/usr/local/bin/recevoir-dump-dispocar",restrict`** — le script est une **copie de
  `recevoir-dump`** (`diff` : chemin de dépôt, motif **`prd|dev-dispocar-*` seulement**, plus
  strict ; commentaire : *« il vit sur L'AUTRE machine … pour être relu et versionné »*). Origine
  **`179.198.198.199`, la même** (42 connexions réussies sur 7 j depuis cette adresse = 18 `vaultbk`
  + 17 `conductorbk` + **7 `dispocarbk`**). Dépôt `/var/backups/dispocar-distant/`
  (`dispocarbk:dispocarbk`, `700`) : **4 dumps `age` de 1 172 octets chacun**, `prd` **=** `dev` à
  l'octet — essais le 09-14 05 h 32 (5 connexions en 37 s : 3 sans dépôt, 2 dépôts), puis
  **01 h 30 (`dev`) et 01 h 32 (`prd`) UTC** cette nuit. **Le bloc `useradd` (VPS-M98) l'a nommé, le
  bloc « non réclamés » (VPS-M99) l'a marqué** *« dépôt écrit par `dispocarbk` — copie d'une AUTRE
  machine »* : la leçon des deux passages précédents a rendu. ⚠️ **Ce que l'audit ne peut pas
  dire** : 1 172 octets pour un `pg_dump | gzip | age`, c'est la taille d'une **base vide** (~600 o
  de schéma gzippé + ~200 o d'en-tête `age`), deux nuits de suite, sur les deux instances. *Soit
  Dispocar n'a pas de données, soit le dump vise la mauvaise base* — **à vérifier côté Dispocar**
  (VPS-M01) : la fraîcheur d'un dépôt mesure l'expéditeur, **sa taille aussi**. ➜ **V30, élargie** :
  (e) reconnaître `dispocarbk` et le rôle de dépositaire d'une **seconde** application ; (f) poser
  la question des 1 172 octets à qui tient Dispocar. `ordonnancement` : **`dispocar-depot-dump`**
  ajouté (01 h 30–01 h 32 UTC, deux connexions = deux instances). ❌ Ne pas lire « 3 comptes de
  service en 5 jours » comme une dérive : même main, même adresse, même modèle de script — **c'est
  le catalogue qui est en retard, pas la machine qui dérive.** Session `vaultbk` : **124 h**, aucune
  commande.
- 🔁 **Vu : 2026-09-14 — LA LECTURE DU 13/09 ÉTAIT À MOITIÉ FAUSSE, ET LES DEUX SCRIPTS LE
  DISAIENT.** Lus ce passage (lecture seule, 2 197 octets chacun, posés le 09-09 à 22 h 27 et
  22 h 41) :

  | Script | Sens | Ce qu'il fait |
  |---|:--:|---|
  | `/usr/local/bin/vault-dump` | **sortant** — un tirage | `sqlite3 "file:…?mode=ro" .backup` (instantané cohérent malgré le WAL) + `tar -czf` de `attachments/`, `rsa_key.pem`, `config.json` dans un `mktemp`, puis `cat` vers la sortie standard — **construit d'abord, envoie ensuite** (une `write error` de tuyau masquerait une vraie troncature, dit le commentaire) |
  | `/usr/local/bin/recevoir-dump` | **entrant** — un DÉPÔT | lit **un** dump chiffré `age` sur l'entrée standard ; nom de fichier **du client, donc non fiable** → `basename` + motif strict `<instance>-<base>-<AAAAMMJJ>-<HHMMSS>.sql.gz.age` (sinon `exit 2`) ; écrit en `.partiel`, vérifie l'en-tête `age-encryption.org` (sinon `rm` + `exit 3`), renomme, `chmod 600` ; **rétention 14 par instance** ; la clé de déchiffrement **n'est pas sur cette machine** |

  **Le dépôt, mesuré** : `/var/backups/vizyo-conductor-distant/` (`conductorbk:conductorbk`, `700`,
  mtime 09-14 01 h 06 min 08) — **14 fichiers** : `prd-vizyo_conductor-*` × 7 (26 284 – 26 694 o)
  et `dev-vizyo_conductor-*` × 7 (30 045 – 30 738 o), du **09-10 00 h 42** (heure de Paris dans le
  nom ; deux paires d'essai à 4 min d'intervalle le soir de la pose) au **09-14 03 h 06** (= 01 h 06
  UTC). **Les « deux connexions par nuit » d'hier sont une par instance** — `prd` puis `dev`,
  01 h 00–01 h 08 UTC, 4 nuits sur 4. Coût : 428 Ko, borné à **~0,9 Mo** par la rétention du script.
  ➜ **Cette machine est le dépositaire hors-site de Vizyo Conductor** — ce que VPS-037 réclame
  pour ses propres sauvegardes, Conductor l'a obtenu ici en une soirée. *Un dépôt hors-site est
  une responsabilité : si Conductor tombe, c'est ici qu'on viendra chercher `prd`.*
  🔑 **Et le collecteur l'avait écrit deux passages de suite** — `vizyo-conductor-distant AUCUNE
  SAUVEGARDE`, ligne 1316 de la collecte du 13/09 — sans que personne ne le lise : son filtre ne
  connaissait que `.gz` et `.gpg`. 🆕 **VPS-M99**, corrigé (`.age`, propriétaire imprimé, troisième
  lecture « dépôt d'une autre machine » dans le bloc des dossiers non réclamés).
  **La session `vaultbk` de 100 h** (`1497860` PPID 1 + `1497933`, `do_poll`, `ESTAB` ←
  `179.198.198.199:50426`) **ne porte aucune commande** : aucun `vault-dump` dessous, pas un octet
  en attente. C'est la **3ᵉ des trois connexions** ouvertes en deux secondes le 09-09 (22 h 22
  min 25 / 26 / 27 — ports 50416, 50424, 50426) ; les deux premières sont fermées. *Une session
  multiplexée (`ControlMaster` + `ControlPersist`) côté Conductor laisserait exactement cette trace
  — l'audit ne peut pas le prouver d'ici (VPS-M01) ; il peut dire que ce n'est **pas** un dump
  bloqué.* Les tirages quotidiens (01 h 32–01 h 41) passent par des sessions **neuves** qui se
  ferment (« `Disconnected from user vaultbk` » à 01 h 41 min 54, 0,6 s après l'ouverture).
  ➜ **V30, reformulée** : (a) reconnaître les quatre objets **et le rôle de dépositaire** ; (b) la
  copie du coffre vit **côté Conductor** (ce que `vault-dump` envoie) — dire qui la relit ; (c)
  fermer ou assumer la session de 100 h ; (d) limite mémoire sur `vizyo-vault` (24 Mo utilisés,
  0 de plafond). ❌ Ne pas ajouter ce dossier à la **couverture** des bases : il n'y a aucune base
  ici à couvrir ; sa fraîcheur mesure la santé de l'**expéditeur**.
...
- **Vu** : 2026-09-15 (1ᵉʳ passage ; l'échec date du **09-14 04 h 30 min 06 UTC**) · **Mesure** :
  ...
  `pairesLocales: 15`, `plusRecente: verify-db_20260913-033110`. Côté poste
  (`D:\sauvegardes\vizyo-verify\copie.log`) : le 09-14 06 h 30 (Paris) le copieur a vu **16
  paires** sur le VPS (15 vivantes + **1 manifeste orphelin**), a commencé par la plus ancienne
  (`20260830`), n'a pas trouvé son archive, et s'est arrêté. **La paire du 09-14 (03 h 31), plus
  loin dans la liste, n'a jamais été atteinte.** Le 09-11, le même mécanisme avait fait
  **re-télécharger** la paire 08-27 (archive encore présente : succès, 3 s) — le signe
  avant-coureur, que personne ne lit.
- **QUOI — la cause : deux rétentions sur les mêmes objets, et un copieur qui s'arrête à la
  première.** (1) Côté VPS, `deploy/vps/backup.sh` purge par **trois `find -mtime +14` séparés**
  (base, fichiers, manifeste). `-mtime +14` tronque à la journée : une archive écrite à
  03 h 31 min 10 et son manifeste écrit 46 s plus tard **ne franchissent pas le 15ᵉ jour à la même
  exécution** quand le timer (`RandomizedDelaySec`) tombe entre les deux secondes → **l'archive
  part, le manifeste reste**. (2) Côté poste, `deploy/local/copie-hors-site.ps1` liste les
  manifestes (`ls -1`, **du plus ancien au plus récent**), et pour toute paire absente localement
  **télécharge ou lève une exception** (`throw "téléchargement de … en échec"`) — la boucle
  entière s'arrête. Sa propre rotation (14 j sur `LastWriteTime`) avait effacé la paire 08-30 le
  09-13 → « absente » → tentative → `throw`. *Deux mécanismes sur le même objet avec deux
  horloges : VPS-003 (deux `pg_dump`) et VPS-041 (deux rotateurs), troisième forme.*
- **`pourquoiInvisible`** : le copieur a **honnêtement** réécrit son fichier d'état avec
  `statut: ECHEC` — et le seul lecteur de ce fichier, le collecteur, **ne jugeait que l'âge de la
  tentative** : `vizyo-verify ECHEC 21 h a jour` (**VPS-M100**). La colonne disait ÉCHEC, le
  verdict disait « à jour », sur la même ligne — et c'est le verdict qu'on lit. Le rapport du
  14/09 a écrit *« VPS-037 : 21 h, 15 copies locales, contenu 22 h — inchangé »*. *Une tentative
  fraîche n'est pas une copie fraîche.*
- **QUOI FAIRE** (**V31**, 10 min côté poste, 0 s sur le VPS) : dans `copie-hors-site.ps1`, **ne
  pas abandonner la boucle** sur une paire dont un fichier manque côté VPS — la tracer (*« paire N
  incomplète sur le VPS, ignorée »*) et continuer ; parcourir **du plus récent au plus ancien**
  pour que la paire de la nuit soit toujours la première servie. Dans `backup.sh`, purger **par
  paire** (un manifeste ne part qu'avec ses deux archives, et inversement) — ou écrire le manifeste
  **avant** les archives. **Gain** : plus jamais une paire de la nuit bloquée par une paire d'il y
  a 15 jours. **Risque** : nul. **Ordre** : le poste d'abord (il ferme le défaut visible), le VPS
  ensuite (il ferme la cause) ; l'inverse laisse le copieur cassé une nuit de plus.
- **Test écrit d'avance, avec sa précondition (VPS-M41)** : le timer de sauvegarde tombe le 09-15
  à **03 h 31 min 27** ; le manifeste 08-30 (16,0 j) est purgé ; le copieur passe à **04 h 30**.
  **Si** `verify_20260831-*.manifest.json` a lui aussi franchi ses 15 jours (une chance sur deux,
  selon les secondes) → le 16/09 rend `statut OK` **et `pairesCopiees ≥ 2`** : *preuve de la
  copie*. **Sinon** → nouvel `ECHEC` sur `verify-db_20260831-…`, `pairesCopiees 0` : *preuve du
  mécanisme*, et la paire du 09-14 reste une nuit de plus sans copie. ⚠️ `statut OK` avec
  `pairesCopiees 0` ne prouverait rien : il faut le **nombre**.
- **`aNePasFaire`** : ❌ ne pas supprimer le manifeste orphelin à la main sur le VPS (écriture ; il
  part seul à 03 h 31 ; et ça masquerait le test). ❌ Ne pas allonger une rétention pour « décaler »
  le problème : deux horloges restent deux horloges. ⚠️ **V6 (second dépositaire, VPS-037) ne
  remplace pas V31** : un second copieur écrit avec la même logique abandonnerait au même
  manifeste.
- **Seuil de réescalade** : gravité 1 si `pairesCopiees` reste à 0 **trois tentatives de suite**
  (la fenêtre de copie locale de 14 j commence alors à se vider de son bord récent) ; `APPLIQUE`
  quand le collecteur rend `statut OK` avec `pairesCopiees ≥ 1` **après** un jour où un manifeste
  orphelin existait sur le VPS (c'est-à-dire : le copieur a *survécu* à une paire manquante).
...
- ✅ **Vu : 2026-09-23 — LES REPÈRES SONT PARTIS À ~103 H, COMME PRÉVU — ET LA PRÉDICTION ÉTAIT FAUSSE D'UNE IMAGE.** Le
  ...
  `until=72h` garantit 72 h, pas plus). (b) : 0 déploiement sur 48 h, rien à prouver.
...
- ✅ **Vu : 2026-09-21 — V32 (a) PROUVÉE : LE REPÈRE DU 19/09 A SURVÉCU AU MÉNAGE DE 00 H 40.** `tracky-api:avant-20260919-1537-1d1521b2`
  ...
  a survécu au même ménage — seconde preuve, gratuite. `docker image prune` de 00 h 40 : section 7 relue,
  `--filter "until=72h"`. **(b)** : *« pas de déploiement depuis le 2026-09-20 : le repli n'a rien à prouver »* — le
  collecteur attend la ligne « repère posé : … ← l'image du conteneur » du prochain `deploy.sh` (après `git pull` sur le
  VPS). Contrepartie mesurée : images 24,34 Go annoncés (chiffre non fiable, VPS-M25), `df` **50 Go / 96**, 46 Go libres —
  rien à payer.
...
- ✅ **Vu : 2026-09-20 — SECONDE MOITIÉ DE LA PREUVE, PAR LA MÊME CHANCE : LA FICHE NE SE FERME PAS.**
  ...
  `tracky-lp:avant-20260919-1826-ad0abff4` = `744ac977a039` du **07/08** (la LP n'avait pas été reconstruite depuis) —
  toutes justes. `avant-20260916-0519` **partie le 18/09 00 h 40** comme prédit (43 h) ; `until=24h` **intact**
  (section 7 relue) ; `deploy.sh` l. 278 : `docker tag "$image:latest" "$image:$etiquette"` — **il étiquette toujours
  `:latest`** ; l'étiquette est juste parce que, deux fois de suite, `latest` n'avait pas été pré-construit. Le seuil
  « deux passages de suite où une étiquette existe et dont l'ID = l'image du conteneur au passage précédent » est
  atteint **par le hasard que (b) doit rendre inutile** : la fiche reste `A_TRAITER`, (b) est le correctif. ⚠️ Le
  collecteur, sous disjoncteur, a rendu *« AUCUNE étiquette avant-* »* — **NON MESURÉ**, mesuré en marge (VPS-M106).
  ⚠️ Le repère du jour meurt **21/09 00 h 40** (32 h) sauf V32 (a). Images : 29 → **31** (`avant-0919` ×3, `latest`
  ×3, `alpine` re-tiré ; `avant-0916` ×2 et 3 autres parties).
- ✅ **Vu : 2026-09-17 — PREMIÈRE MOITIÉ DE LA PREUVE, ET UN SEUIL QUI N'EST DÉCIDABLE QU'AVEC V32 (a).**
  `tracky-api:avant-20260916-0519-0c9672c9` = **`cc1fec4a6ebb`** (créée 09-15 17 h 45 min 25),
  `tracky-web:avant-…` = **`c8d99972e2eb`** — exactement les images que portaient les conteneurs au passage
  du 16/09. L'étiquette est juste parce que `latest` n'avait **pas** été pré-construit : `deploy.sh`
  (`87ae29c9`, 05 h 24 min 17, **292 s** — 2ᵉ durée à deux cœurs) a étiqueté ce qui tournait. **Mais
  `until=24h` est intact** : étiquetée 05 h 19 le 16/09, elle aura **43 h au ménage du 18/09 00 h 40** →
  supprimée, sauf V32 (a) ou nouveau déploiement. *Le seuil « deux passages de suite » est donc indécidable
  sans V32 (a)* (VPS-M94 : une source qui disparaît avant l'échéance) — réécrit **« deux passages de suite
  où une étiquette existe »**, et V32 (a) devient la condition de la **mesure**. 🆕 Le collecteur porte
  `imagesProdListe` (`tracky-api=80d613258b7a,tracky-web=9b960ba4b17f`) et vérifie **l'arrière** à chaque
  passage (angle mort n° 1 du 16/09, traité ; première pose ce matin). Images **32 → 29** : `avant-1731` ×2
  **et** `avant-1741` ×2 (31 h) + `alpine` partis, `latest` api/web arrivés — la prédiction « 30 ou 32 »
  oubliait les 1741.
...
- ✅ **Vu : 2026-09-23 — JOUR 3 / 7, ET UNE CORRECTION DE SÉRIE (VPS-M114).** `sar -u` lu **avec l'en-tête** : 22/09 steal
  **3,39 %** moyen, **max 7,95 %** (17 h 20), idle **86,07 %** ; 21/09 steal **2,76 %** moyen — **et non 0,24 %** comme écrit
  ...
  Pendant la collecte : steal 6,7 % (fenêtre de 172 s, normal).
...
- ✅ **Vu : 2026-09-21 — PREMIER JOUR ENTIER SANS LIMITATION : STEAL 0,1–0,3 % SUR 34 RELEVÉS DE SUITE.** `sar` 20/09
  ...
  09-20 après le reboot **87,13 %** d'idle, 09-21 partiel **87,79 %** — contre 31,35 % le 09-19. `dockerd` 1,0 %, 0 client,
  charge 0,20, `docker ps` 0,14 s, `/api/health` **33 ms**. *Le test écrit d'avance (« fenêtre glissante ou limite qui ne se
  lève pas seule ») est sans objet : elle a été levée à la main, et n'est pas revenue.* ⚠️ **Mais l'avertissement de
  l'hébergeur a été mis à l'épreuve 50 min après la levée** : 14 h 20 → 16 h 22, une boucle du poste a ouvert **7 774
  sessions SSH** (`user`+`sys` 10 → 23 %, `proc/s` ×4,2 — **VPS-047**) ; `steal` est resté à 0,2–1,5 % pendant ces deux
  heures : *la politique ne s'est pas ré-appliquée pour 23 % de CPU sur 2 h — le seuil est au-dessus, sans qu'on sache
  où.* Seuil inchangé : `APPLIQUE` après 7 jours de steal < 10 % en moyenne journalière (jour 1 / 7).
...
- **Vu** : 2026-09-20 (1ᵉʳ passage ; le premier palier date du **19/09 22:10 UTC**) · **Mesure** — `sar -u`,
  ...
  **32,5 %** (00:10) → **11,3 %** (01:10) → **9,4–10,7 %** de 02:00 à 05:20 → **15,3 %** (05:30) → **18,1–24,6 %** de
  05:40 à 11:10 (plateau plat, 34 relevés). `vmstat 1` à 11:14 : `us 6 sy 4 id 0 st 89–91`, `r` 45–49 ; charge
  13 → 58 ; `dockerd` **0,9–3 %**, **0 client `docker`**, cumul figé 395,5 h — *la cause côté VM (VPS-016 nº 6 et
  nº 7) est éteinte depuis ~05:25*. Production : 200 en 0,5–2,1 s.
- **QUOI — ce que l'audit peut dire** : les paliers tombent **à :10 de chaque heure**, chacun retire ~un quart de ce
  qui reste ; les plateaux sont plats à ±1 point (89,3–90,6 % pendant 4 h 10, 75,4–82,4 % pendant 5 h 40) ; le
  déclencheur est daté (2 × 100 % de CPU à partir de 18:40, premier palier 3 h 30 plus tard) ; la sonde horaire de
  l'hébergeur lit `ps --sort=-%cpu` à :35 et `usage-telemetry.py` à :15 (VPS-027). **Ce qu'il ne peut PAS dire** :
  *pourquoi* — limitation d'équité (l'hypothèse que dessinent les paliers), fenêtre glissante, voisin, maintenance.
  **Ça se lit côté hôte.** ⚠️ La règle du 08-10 — *« un steal élevé ne prouve pas une contention d'hôte : une machine
  saturée en déclare mécaniquement plus »* — était vraie **pendant** la boucle ; elle ne couvre pas ce cas :
  `dockerd` à 1 %, de l'idle (10–15 %), et le steal tient. *Une règle écrite sur un cas encode ce cas (VPS-M49).*
- **Ce que ça a coûté, mesuré** : reprises du 20/09 (30 `RESTORE SCHEDULER`) : **11 ≤ 10 s, 16 en 10–60 s, 1 en
  60–180 s, 2 à 220 s** — la veille 30 / 30 ≤ 10 s ; **2 passées par le secours SMS** (`channel=SMS`, 3 puis 2
  tentatives) — TRK-083 / T71 exercé, facturé ; sauvegarde Tracky **64 min** (37 s la veille), 3 min 59 de CPU
  (22 s) ; `trip_automation_runs` 01h–03h **11 · 12 · 18 min** contre 3 · 1 · 5 à la même heure la veille
  (VPS-M103 tenu) ; `logrotate` 3 min 40 de mur ; `dockerd` : *« Health check … timed out starting »* ×96 à 01 h,
  ×213 à 05 h — les sondes n'obtiennent plus le CPU pour démarrer ; Vizyo Auth injoignable 00:00–00:02 ; la
  collecte de l'audit 647 s, charge 15,8 → 81,7 (VPS-M107).
- **`pourquoiInvisible`** : rien ne surveille le CPU *servi* — `/admin/system` montre la charge (58) sans dire que
  90 % des cycles ne sont pas donnés ; le centre d'alerte a vu l'effet (503 en 57 s) et nommé la cause visible (les
  deux clients), sans pouvoir voir que l'hôte ne suivrait pas ; le propriétaire a été réveillé par « Tracky est
  down » 7 h après le premier palier, cet audit est arrivé 13 h après (angle mort « rien entre deux passages »,
  36ᵉ report).
- **QUOI FAIRE — V35** : (1) **ticket hébergeur** avec l'extrait `sar -u -f sa19 -s 18:00 ; sar -u -f sa20` : *« une
  limitation de CPU est-elle appliquée à srv1201617 depuis le 19/09 22:10 UTC ? règle ? levée ? la cause interne est
  éteinte depuis le 20/09 05:25 »* — gain : la seule réponse qui dit si le CPU reviendra ; (2) **le seul levier
  interne** : `docker compose stop` (jamais `down`) sur les **18 conteneurs hors production** (`maalem-dev-*` ×6,
  `maestroo-dev-*` ×4, `tracky-demo-*` ×4, `dronely-*` ×2, `foodsqan-maquettes`, `dg-epaviste-website`) — leur part
  des 0,4 vCPU et de ~72 sondes/min revient à Tracky, Auth, Texto, Vault ; contrepartie : dev et démo indisponibles ;
  risque nul pour la production ; ⚠️ **`foodsqan-traefik` reste** (VPS-021) ; à mesurer à heure égale ; (3) **ne rien
  ajouter** : ni déploiement, ni build, ni `du`, ni V4.
- **Test écrit d'avance (VPS-M41, avec précondition)** : *si* la politique est une fenêtre glissante de 24 h, la part
  servie remonte par paliers entre le 20/09 22:10 et le 21/09 05:30 sans geste ; *si* elle est encore à ~21 % au
  passage du 21/09 02:20, c'est une limite qui ne se lève pas seule → le ticket est la voie. Précondition : aucun
  build ni `up --build` d'ici là.
- **`aNePasFaire`** : ❌ ne pas lire « steal 88 % » comme « la boucle `dockerd` continue » — c'est un autre constat ;
  ❌ ne pas conclure de la coïncidence 05:25 / 05:30 (10 → 21 %) que tuer les clients a rendu la moitié du CPU — deux
  points, puis six heures de plateau ; ❌ pas de `docker system df`, `docker stats`, `du /opt`, ni build « pour
  voir » : chaque commande prend dix fois son temps **à la production** ; ❌ pas de redémarrage (V4) : il ne rend pas
  ce que l'hôte retient et coûte ~5 min de coupure à ce régime.
- **Seuil** : `SURVEILLANCE` gravité 2 quand la part servie est **> 90 % sur un relevé `sar` entier** (24 lignes
  d'une journée) ; gravité 1 tant qu'elle est < 50 %.
...
- ✅ **Vu : 2026-09-21 — LA DÉMO EST BIEN SUR LES IMAGES COURANTES, ET LE COLLECTEUR LE VOIT SEUL.** Section 4 :
  ...
  ajouté 578 109 positions, 15 671 trajets, 12 967 analyses, `recalé 09-20 13:47`) ; `journalctl -u tracky-demo-refresh`
  (banc VPS-M108) : **« Deactivated successfully » 13:47:16** — le `Result` en mémoire est parti avec le reboot, le journal
  le garde. ⚠️ La section 5 lit sur cette base *« 🔴 SILENCE : 37 émetteurs n'ont plus émis depuis 19 h 00 »* et *« 11
  émetteurs ont disparu »* : **c'est l'import** (VPS-M101 — les horodatages sont ceux de la source, la démo n'ingère rien
  en direct), pas une panne. (b) toujours **en attente du prochain `git pull` + déploiement** ; preuve finale au dimanche
  27/09 04 h 00.
...
- **Vu** : 2026-09-20 (1ᵉʳ passage ; l'échec date du **20/09 04:07:58 UTC**) · **Mesure** :
  ...
  *« ❌ Import de la démo en échec : Invalid `prisma.fleet.upsert()` invocation: **The column `managedByManagerAt`
  does not exist in the current database** »*, *« IMPORT EN ÉCHEC — la base de démo est restée dans son état
  précédent »* 04:07:58, *« API de démo redémarrée »* 04:08:38 — **8 min 35 d'API démo arrêtée** pour rien ; le
  13/09 le même passage avait réussi en 4 min 30. `tracky_demo` **581 Mo** ; données de la démo figées au **13/09**.
- **QUOI — la cause : deux images, un schéma.** `docker-compose.demo.yml` (en-tête) : *« les MÊMES IMAGES que la prod
  … un `up -d` sur ce fichier recrée les conteneurs de la démo sur ces images et joue les migrations. La démo ne
  peut pas être en retard sur la prod autrement que par l'oubli de cette seule commande »*. **`deploy.sh` ne lance
  pas cette commande** (relu : `up`, `up-lp`, jamais la démo). La production a été déployée trois fois depuis (17/09
  13:37 `1ba0cc25`, 15:37 `1d1521b2` — **le lot D, qui ajoute `managedByManagerAt`** —, 19/09 16:36 `8e289f38`) ;
  `tracky-demo-api` n'est pas dans les conteneurs recréés depuis 48 h et tourne sur une image d'avant ; l'importeur,
  lui, est `image: tracky-api:latest` — le code du 19/09 — et parle à une base au schéma du 13/09. **Le CPU (VPS-045)
  n'est pas la cause** : il n'a fait que ralentir l'échec (5 min 26 au lieu de ~2).
- **`pourquoiInvisible`** : le passage est **hebdomadaire** — le décalage date du 17/09, son premier effet mesurable
  est ce matin ; le compose le savait et l'a écrit, personne ne relit un en-tête de compose en écrivant `deploy.sh` ;
  le bloc « unités en échec » de la section 7 l'a rendu en une ligne.
- **QUOI FAIRE — V36** : (a) sur le VPS, hors HH:42–HH:46 et **quand VPS-045 est levé** (un `up -d` recrée deux
  conteneurs et joue `prisma migrate deploy` — ~1 min à deux cœurs) : `cd /opt/vizyo-tracky/deploy/vps && docker
  compose --env-file .env.demo -f docker-compose.demo.yml ps` (relire) puis `… up -d` ; gain : démo à jour, import du
  27/09 réussi ; contrepartie : ~1 min de démo ; risque nul pour la production (réseaux séparés, l'importeur seul
  touche `vizyo-tracky` en lecture). (b) **dans `deploy.sh`** : après `up`, un `up -d` sur la démo — ou une garde qui
  refuse de laisser la démo à plus d'un déploiement de retard. **Preuve** : `journalctl -u tracky-demo-refresh` du
  27/09 = *« Import réussi »*, et `tracky-demo-api` dans « conteneurs RECRÉÉS » le lendemain de chaque déploiement.
- **`aNePasFaire`** : ❌ ne pas relancer `tracky-demo-refresh-demande` « pour voir » (même importeur, même échec,
  8 min d'API démo arrêtée de plus) ; ❌ ne pas jouer la migration à la main dans `tracky-demo-postgres` (incident du
  17/09) ; ⚠️ V27 (la démo n'analyse pas) n'est pas concernée ; ⚠️ V35 (2) arrête la pile démo : faire (a) **après**
  le `start`, pas avant.
- **Seuil** : `APPLIQUE` au premier import hebdomadaire réussi **après** un déploiement de production sans geste
  manuel sur la démo.
...
- ✅ **Vu : 2026-09-23 — JOUR 2 / 7 : LA VEILLE A PORTÉ 229 SESSIONS, POINTE 15 H = 56, ET LE BLOC M112 L'A DIT SEUL.**
  *« ✅ la VEILLE (2026-09-22) : 229 sessions, heure de pointe 15h=56 — sous le seuil V37 (600/h) »*. Ventilé à la main par
  ...
  passage** des agents `VizyoTracky-*` du poste (VPS-M115) + 5 dépôts de Conductor / Dispocar / coffre. `sar -w` 22/09 :
  `proc/s` **27,7** moyen (27,0–41,5). Aucun `Monitor` relu : la 2ᵉ moitié de la preuve attend toujours une occasion.
...
- **Vu** : 2026-09-21 (1ᵉʳ passage ; la rafale date du **20/09 14 h 20 → 16 h 22 UTC**) · **Mesure** — `auth.log`, lu par le
  ...
  |---|---|---|
  | sessions SSH réussies le 20/09 | **7 946** (record : 4 252 le 08-20, 935 le 17/09, 121 la veille) | section 6, par jour |
  | dont depuis le poste (`82.67.153.51`, clé `vizyo-vps-hostinger`, compte root) | **7 941** ; les 5 autres = Conductor / Dispocar / coffre | VPS-M110 |
  | par heure UTC | 14 h **2 563** · 15 h **3 894** · 16 h **1 317** — le reste de la journée **≤ 29 / h** | VPS-M110 |
  | par tranche de 10 min | 14:1x **4** → 14:2x **589** → 579–696 pendant 2 h → 16:1x 470 → 16:2x 194 → 16:3x **6** | marge |
  | pic par minute | **76** (15:47), 74, 73 — soit **une session toutes les 0,8 s** | marge |
  | durée d'une session | ouverture → `Removed session` en **0,6–1,0 s**, la suivante 0,6 s après (échantillon 15:00:00–15:00:06, sessions 47699–47702) | `systemd-logind` |
  | **effet mesuré** (`sar -w`, `sar -u`) | `proc/s` **28 → 117** (×4,2 : 14:40 → 16:00), `cswch/s` 3 000 → 5 800, `user`+`sys` **10 → 23 %**, idle 82 → 70 %, `runq` jusqu'à 26 | `sa20` |
  | fin | la rafale **s'arrête avec le redémarrage V4** (16:17–16:20 ; 194 sessions encore en 16:2x, puis 6) | marge |

  À ~54 processus par session (plancher du 22/08), c'est **~420 000 processus en deux heures**, soit **quatre fois la
  charge de fond de la machine** pendant ce temps — et `proc/s` le confirme sans passer par le plancher.
- **QUOI — la cause, IDENTIFIÉE ce passage dans les transcriptions du poste (pas déduite)** : la session Claude Code du
  20/09 après-midi (`87c19204`, celle qui a fait V29 et V4) a lancé **quatre outils `Monitor` de 30 min à la suite** —
  14 h 21 min 03, 14 h 51 min 26, 15 h 22 min 54, 15 h 53 min 45 UTC — chacun de la forme
  `while true; do r=$(ssh root@72.62.26.240 "… psql … count(*) from trip_automation_runs where status='running'"); …; sleep 30; done`
  (« fenêtre calme : aucun passage d'automatisation en cours »). **Dans le shell de `Monitor`, `sleep` et `date` ne sont
  pas dans le `PATH`** : `sleep 30` échoue instantanément (*command not found*, avalé), et la boucle tourne **à la vitesse
  d'un aller-retour SSH** (0,9 s). **Preuve** : le 4ᵉ guetteur journalisait dans `guetteur.log` — **1 751 lignes en 30 min**
  (58/min), et son préfixe `$(date -u +%H:%M:%S)` est **vide** sur chaque ligne (`« min=53 running=1 »`) : `date` n'a pas
  été trouvé, donc `sleep` non plus. Le 5ᵉ `Monitor` (16 h 17 min 35, « retour SSH après redémarrage ») écrit
  `S=/usr/bin/sleep; D=/usr/bin/date` **en toutes lettres** — l'auteur l'avait vu, après deux heures. Les quatre fenêtres de
  30 min mises bout à bout font **14 h 21 → 16 h 23**, à la minute près la rafale mesurée (14:2x → 16:2x, 194 sessions
  dans la dernière tranche, 6 après). *Ce n'était pas une décision de cadence : c'était un `sleep` qui ne dormait pas.*
- **Ce que ça coûte, et pourquoi c'est gravité 2 et pas 3** : (1) à 13 h 36 le même jour, Hostinger a levé une limitation de
  CPU en écrivant *« may be reapplied after sustained usage »* — **50 minutes plus tard**, la VM passait de 10 à 23 % de
  `user`+`sys` pendant deux heures pour ne rien produire ; (2) `sshd` + PAM + `logind` + `systemd --user` par session sont
  des **forks de l'hôte**, pas des conteneurs : `nice` n'y peut rien, et `docker-orphelins.timer` ne les voit pas ; (3) le
  coût est **invisible à tout ce qui surveille** : `/admin/system` montre une charge de 0,6–1,3, le centre d'alerte n'a rien
  vu, et ce collecteur n'a compté que le NOMBRE jusqu'à ce passage.
- **`pourquoiInvisible`** : VPS-032 comptait les sessions **par jour** depuis le 22/08 et disait « ~600/j, ce sont les
  agents » ; le compteur par jour lit 7 946 et 935 sur la même échelle (« beaucoup »). Il fallait **l'heure** pour voir que
  7 774 des 7 946 tiennent en deux heures — et l'heure n'était pas ventilée (angle mort n° 4, 10ᵉ report, traité ce
  passage : VPS-M110). *Un total journalier ne distingue pas un plateau d'une rafale.*
- **QUOI FAIRE — V37** : (1) ✅ **identifier la boucle** — fait ce passage (ci-dessus) ; (2) **la règle, là où les commandes
  s'écrivent** (`CLAUDE.md` et la mémoire des agents, à côté de la règle `timeout` de V34) : *« dans un `Monitor` ou toute
  boucle du poste, `sleep` et `date` s'écrivent `/usr/bin/sleep` et `/usr/bin/date` — un `sleep` introuvable ne dort pas,
  il fait tourner la boucle à la vitesse du réseau ; et une surveillance du VPS = UNE session qui boucle côté serveur
  (`ssh root@… 'while …; do …; sleep 20; done'`), jamais un `ssh` par itération »* ; un garde en tête de boucle :
  `command -v sleep >/dev/null || exit 9` ; (3) ✅ **le seuil au collecteur** (posé ce passage, VPS-M110 : la ventilation par
  heure du jour de pic) — 🔴 dès qu'une heure porte **> 600 sessions** (10/min) : c'est le pic du 08-20 (4 252 / j sur
  ~14 h ≈ 300 / h) multiplié par deux, donc au-dessus de toute journée de travail connue.
  **Gain** : 4× moins de forks pendant une surveillance, et surtout **ne pas re-déclencher** la politique de l'hébergeur
  le jour même où elle a été levée. **Risque** : nul. **Contrepartie** : aucune — une session persistante rend les
  mêmes lignes, moins cher. ⚠️ **Hors périmètre d'écriture de cet audit** pour (2) : `CLAUDE.md` est du code applicatif ;
  la mémoire du poste, elle, est écrite ce passage.
- **`aNePasFaire`** : ❌ ne pas bannir `82.67.153.51` ni limiter `MaxStartups` : c'est le poste, et brider `sshd` bride
  aussi `deploy.sh` et cet audit. ❌ Ne pas lire les **5 sessions** de Conductor/Dispocar/coffre dans le même sac : elles
  sont à leur heure et à leur cadence (VPS-042). ❌ Ne pas attribuer le `user`+`sys` de 14 h–16 h à V29 ou à la démo (V36
  (a) : 13 h 40–13 h 47, import terminé avant la rafale) — `proc/s` monte **avec** les sessions, à la minute.
- **Seuil de réescalade** : gravité 1 si une rafale > 600 sessions/h coïncide avec un `steal` > 30 % (l'hébergeur ré-applique) ;
  `SURVEILLANCE` quand 7 jours passent sans heure > 600 **et** que la règle est écrite ; `APPLIQUE` quand la boucle est
  nommée et corrigée.
...
- **Vu** : 2026-09-23 (1ᵉʳ passage ; le conteneur date du **22/09 13 h 53 min 44 UTC**) · **Mesure** :
  ...
  | Grandeur | Valeur | Source |
  |---|---|---|
  | conteneur | `cdef31-site`, `nginx:alpine`, `unless-stopped`, projet compose `cdef31-vizyoagency` (dossier `/opt/cdef31-vizyoagency`) | §4 — VPS-M92 : *« 🟠 NEUF(S) depuis le manifeste du 2026-09-22 : cdef31-site »* ; `docker inspect` sous `timeout 20` |
  | service | `cdef31.vizyoagency.com`, routeur Traefik `cdef31`, certificat ACME (`acme.json` réécrit 22/09 16 h 20) | §4 |
  | contenu | `public/` **21 Mo** — 3 pages statiques, polices auto-hébergées, 12 vidéos (20 Mo) ; montages **en lecture seule** ; *« aucun backend, aucun build »* | `du`, compose, `README.md` |
  | coût | **3,5 Mio** de RAM, 0,00 % CPU, 3 processus | `docker stats --no-stream` |
...
- **QUOI — la cause** : le site est posé **et retouché sur le serveur** — le compose le prévoit (*« Mise à jour de la page :
  ...
  la machine (elles visent des bases, pas des dossiers).
- **`pourquoiInvisible`** : le conteneur ne l'a pas été — c'est le premier rendement de VPS-M92 depuis sa pose (05/09).
  L'**écart de version**, lui, ne se voit qu'en comparant des dates de fichiers de part et d'autre : la page répond, le
  conteneur est sain, rien ne dit que la copie locale a quatre heures de retard.
- **QUOI FAIRE** : (1) **rapatrier et versionner** — `scp -r root@72.62.26.240:/opt/cdef31-vizyoagency D:/www/vizyo-agency/cdef31-vizyoagency`
  puis `git init` + commit (30 s, lecture seule côté VPS) ; les retouches suivantes se font localement puis `scp`. (2) *(au
  prochain passage sur ce projet)* `mem_limit: 32m` dans son compose. **Pas de sonde** : pour un nginx sans backend, Traefik
  voit déjà la panne (502), et une sonde à 30 s ajouterait ~2 invocations/min à la 1ʳᵉ charge de fond (VPS-011).
  **Gain** : la seule version à jour d'un livrable client existe hors du disque audité. **Contrepartie** : 21 Mo sur le poste.
- **`aNePasFaire`** : ❌ ne pas passer par `docker cp` / `docker exec` (le contenu est un montage de `/opt` ; V34). ❌ Ne pas
...
- **Vu** : 2026-09-23 · **Mesure** : le bloc « pendant MA collecte » a écrit *« 🟠 UNE AUTRE SESSION que celle qui exécute ce
  ...
  **cinq** tâches `VizyoTracky-*` (`outils/taches-planifiees.ps1`) dont le script appelle `ssh` (`outils/agent-*.cjs`) :
  `RecitTrajet` (03 h 15 Paris), `RattrapageRecits` (toutes les 2 h), `CourrierIA` (06 h 30 / 14 h 30), `LimitesVitesse`
  (04 h 30, 08 h 30, 14 h 00, 18 h 30, 22 h 00), `QualiteGPS` (05 h 00). **Aucune** n'était au catalogue. Coût mesuré sur
  `auth.log` du 22/09 par heure × clé : **~3 sessions par passage**.
- **QUOI** : le catalogue a été construit **depuis le VPS** (crons, timers, dépôts entrants) et le poste n'y figurait que par
  les tâches **de l'audit** — l'outil décrivait son propre ordonnancement et celui du serveur, pas celui de tous ceux qui lui
  parlent. C'est VPS-M57 (collision audit ↔ audit) à l'échelle du poste : sans ces entrées, `LimitesVitesse` à 02 h 30 UTC
  entre dans **chaque** passage d'audit qui part avec plus de 8 min de retard.
- **Correctif** : 5 entrées `couche: "Poste → VPS"` dans `ordonnancement`, avec heure locale ET UTC, cadence, coût mesuré.
  ⚠️ **Échéance datée** : au passage à l'heure d'hiver (**25/10**), toutes ces heures (et celle de l'audit) glissent d'une
...
- **Domaine** : méthode · **Gravité** : 3 · **Statut** : `APPLIQUE` (2026-09-23 — corrigé à la source : VPS-045, passage du 22/09)
- **Vu** : 2026-09-23 · **Mesure** : `LC_ALL=C sar -u -f /var/log/sysstat/sa21`, ligne `Average` :
  `%user 5.72 · %nice 0.01 · %system 4.23 · %iowait 0.24 · %steal 2.76 · %idle 87.03`. Le rapport du 22/09 (verdict,
  tableau, § 2, VPS-045) a écrit **« steal 0,24 % de moyenne sur les 143 relevés (max 2,78) »** : la 4ᵉ colonne prise pour
  la 5ᵉ, et le « max » était la moyenne du steal. 22/09 : steal **3,39 %** moyen, **max 7,95 %** (17 h 20). La section 9 du
  collecteur était juste (`steal% 2.76`).
- **QUOI** : une lecture **à la main, en marge**, d'une sortie `sar` coupée de son en-tête (`tail -1`). Le chiffre était
  vrai — il n'était pas celui qu'on nommait. Famille de VPS-M01.
- **Conséquence** : aucune sur le verdict (3 % n'est pas 80–90 %, VPS-045 garde son « jour N / 7 » sous le seuil de 10 %) ;
  une série fausse d'un facteur 10 dans deux rapports et dans la fiche.
- **Correctif** : toute lecture `sar` en marge se fait **avec l'en-tête** (`… | head -3` et `tail -1`), ou par la section 9.
  La clé `steal24hPct` du manifeste porte le **maximum** de la veille (son sens depuis sa pose), jamais la moyenne.
- **`aNePasFaire`** : ❌ ne pas réécrire le rapport du 22/09 : il reste tel qu'il a été publié, et cette fiche le corrige.

### VPS-M113 — La charge de départ de la collecte était un nombre sans auteur

- **Domaine** : méthode · **Gravité** : 4 · **Statut** : `APPLIQUE` (2026-09-23 — bloc « au DÉPART » posé, bancé deux fois)
- **Vu** : 2026-09-23 (angle mort n° 9 du 22/09) · **Mesure** : le 22/09, départ à **1,09** alors que `sar` lisait 0,14 à
  04 h 30 ; le bloc BUDGET a conclu *« 🟠 CHARGE PARTAGÉE … cause non nommée »*, et `sar`/`pidstat` ne se relisent pas à la
  seconde a posteriori.
- **Correctif** : **avant toute autre commande**, une photo : compte des états `R` et `D` (ceux que `loadavg` additionne) et
  les 8 processus **nés dans les 5 min**, hors ma session (`$$`, son parent, < 10 s) et hors threads noyau, **triés par %CPU** ;
  imprimée dans le bloc BUDGET sous « ── au DÉPART (VPS-M113) ». **Coût** : 2 `ps`, 0 docker. **Banc** sur la machine : la
  1ʳᵉ version, triée par âge, rendait des threads noyau et des `postgres idle` (vrai, inutile) → retriée par %CPU ; à un
  instant calme elle rend des connexions à 0,3 % — c'est la bonne réponse.
- **Portée écrite** : un processus **déjà mort** au départ n'y figure pas — la photo nomme ce qui tourne encore, pas tout ce
  qui a tourné. Elle ne remplace pas `sar`, elle lui donne un nom.
...
- **Vu** : 2026-09-21 · **Mesure** : le bloc « Sessions SSH RÉUSSIES » (VPS-032, 22/08) rend un total 7 j et un
  ...
  19/09 l'avait approché par déduction (la chute 935 → 132 au moment de la pause des agents), jamais par mesure.
- **QUOI** : tout ce qu'il fallait était déjà dans `$ECH` — la ligne `Accepted publickey for <compte> from <ip> port <p>
  ssh2: <type> SHA256:<empreinte>` porte le compte, l'origine et la clé, et son horodatage porte l'heure. Le bloc les
  lisait pour le top 5 des IP et pour l'inventaire des clés (VPS-M77), puis les **jetait** au moment de compter.
- **Correctif** : sur le **jour de pic** de la fenêtre (le seul qui vaille une lecture) : sessions par `compte depuis
  <ip>` (top 4), par empreinte **nommée** via l'inventaire `authorized_keys` déjà lu (top 4), et un histogramme **par
  heure UTC** (une barre = 20 sessions). **Zéro commande de plus** : trois `grep`/`awk` sur une variable en mémoire.
  **Banc** sur la machine : le bloc a rendu le tableau ci-dessus en une passe, la clé `vizyo-vps-hostinger` nommée, les
  5 sessions externes attribuées à leurs comptes.
- **`aNePasFaire`** : ❌ ne pas lire « 7 941 depuis le poste » comme « 7 941 de Claude » — l'IP et la clé sont celles du
  **poste**, partagées par tout ce qui y tourne (VPS-M01) ; ❌ ne pas ventiler **tous** les jours : sept histogrammes de
  24 lignes noieraient le seul qui compte.

### VPS-M109 — Le bloc `alpine` comparait un âge lu sur la machine à un seuil de 24 h écrit dans le script, le lendemain du jour où le cron est passé à 72 h

- **Domaine** : méthode · **Gravité** : 3 · **Statut** : `APPLIQUE` (2026-09-21 — seuil lu dans `/etc/cron.d/docker-image-prune`, banc)
- **Vu** : 2026-09-21 · **Mesure** : section 11, bloc VPS-026 : *« présent, tiré localement le 2026-09-19 03:31:33 — soit
  il y a 46 h · 🔴 PLUS DE 24 h : le ménage de 00 h 40 le supprimera »*. Or le ménage de 00 h 40 venait de **l'épargner**
  (`until=72h` depuis V32 (a), 20/09 13 h 12), et l'image était bien là à 02 h 25.
- **QUOI** : la grandeur qui gouverne (l'âge) venait du VPS ; le seuil auquel on la compare était **une constante du
  script**, posée le 16/08 quand le cron disait 24 h. Un réglage du VPS a changé, et le collecteur a continué à raisonner
  sur le souvenir du réglage. C'est la classe de VPS-M55 (*une attribution écrite en dur, republiée chaque nuit*) — et
  le bloc VPS-M92 « fraîcheur du catalogue » avait justement imprimé *« 🆕 /etc/cron.d/docker-image-prune modifié il y a
  0 j »* deux sections plus haut, sans que le bloc `alpine` s'en serve.
- **Correctif** : `PRUNE_UNTIL_H` lu par `grep -oE 'until=[0-9]+h'` sur le cron ; comparaison contre **lui** ; sursis
  restant imprimé ; si le cron est illisible → *« seuil NON LU : aucune prédiction »* (VPS-M02), jamais un repli muet sur
  24 h. **Banc** : `PRUNE_UNTIL_H=72`, âge 46 h → *« ÉPARGNÉ … encore ~26 h de sursis »*. ⚠️ La **période** de VPS-026
  change avec le seuil : 2 jours à 24 h, **4 jours à 72 h** (tiré 19/09 03:31 → supprimé 23/09 00:40 à 93 h → re-tiré
  23/09 03:31). *Prédiction écrite d'avance, à lire le 23/09.*
- **`aNePasFaire`** : ❌ ne pas « corriger » en écrivant 72 à la place de 24 : ce serait le même défaut avec un autre
  chiffre.

### VPS-M108 — Le lendemain d'un redémarrage, dix unités « JAMAIS EXÉCUTÉES » dont les six sauvegardes de la veille — parce que `ExecMainExitTimestamp` vit en mémoire et que le journal, lui, survit

- **Domaine** : méthode · **Gravité** : 2 · **Statut** : `APPLIQUE` (2026-09-21 — lecture du journal persistant quand l'horodatage est vide, banc sur 8 unités)
- **Vu** : 2026-09-21 · **Mesure** : sections 7 et 11 : *« ⬜ tracky-backup.service JAMAIS EXECUTEE — aucun résultat à
  lire »*, idem `vizyo-verify-backup`, `vizyo-auth-backup`, `vizyo-manager-backup`, `vizyo-texto-backup`,
  `capcom6-backup`, `tracky-demo-refresh`, `apt-daily-upgrade`, `e2scrub_all`, `update-notifier-motd` — **10 unités sur
  26**. Dix lignes plus bas, la section 11 listait `tracky_prod_20260920-030106.sql.gz` (22 h, relu, complet) et les six
  archives de la veille. Le banc sur la machine : `journalctl -u tracky-backup.service --since -7d` → *« 2026-09-20T04:05:08
  Deactivated successfully »* ; `apt-daily-upgrade` → 06:22:25 ; `tracky-demo-refresh` → 13:47:16 (l'import réussi de
  V36 (a)) ; `update-notifier-motd` → **rien sur 7 j** (le seul vrai « jamais »).
- **QUOI** : `systemctl show -p ExecMainExitTimestamp` est l'état **en mémoire** de systemd ; le redémarrage V4 (16 h 17)
  l'a vidé pour toute unité qui n'a pas encore tourné depuis le boot. Le bloc VPS-M87 (07/09) avait été écrit pour ne plus
  dire « succès » sur une unité jamais partie ; le lendemain d'un reboot il dit **« jamais partie » sur une unité qui a
  tourné la veille** — VPS-M09 (*un jour de redémarrage rend l'historique illisible*) sous une forme nouvelle, et
  exactement le genre de vide que VPS-M02 interdit : dix ⬜ qui se lisent « rien ». Le passage était prévenu (post-scriptum
  du 20/09 : *« uptime < 1 j … ne pas y lire une chute »*) — c'est le **collecteur** qui ne l'était pas.
- **Correctif** : `_dernier_run_journal <unité>` — `journalctl -u … -q -o short-iso --since -7d | grep -E 'Deactivated
  successfully|Failed with result' | tail -1`, appelé **seulement** quand l'horodatage est vide (sur un jour sans reboot :
  ~5 hebdomadaires ; le lendemain d'un reboot : toutes). Trois verdicts au lieu d'un : *« pas encore tournée DEPUIS LE
  REDÉMARRAGE (uptime N h) — dernière fin au journal : <date> succès|ÉCHEC »*, *« JAMAIS EXÉCUTÉE — ni résultat, ni
  aucune trace au journal sur 7 j »*, et la branche VPS-015 inchangée. Dans les deux blocs (7 et 11). **Coût** : 411 ms
  pour 8 unités au banc — lecture d'index, pas de disque.
- **`aNePasFaire`** : ❌ ne pas retirer le verdict « jamais exécutée » : il reste juste quand le journal est vide, et c'est
  le mode d'échec de VPS-015 ; ❌ ne pas lire « succès au journal le 20/09 » comme « la sauvegarde du 21/09 est faite » —
  elle tombe à 03 h 01, après ce passage ; la fraîcheur se lit dans le bloc « âge par dossier », pas ici.
...
- 📏 **Vu : 2026-09-21 — MODE COMPLET (steal 0 % au départ), 149 s, charge 0,20 → 1,93 : LE MODE MINIMAL N'A PAS ÉTÉ
  EXERCÉ, ET C'EST LA BONNE NOUVELLE.** *« steal au départ : 0 % (mode complet ; le mode allégé s'enclenche à 50 %) »* ;
  aucune clé `NON MESURÉE` au manifeste. Le 2ᵉ jet reste **à confirmer au premier vrai passage sous steal ≥ 80 %** —
  qu'on espère ne pas voir. ⚠️ Rien n'a été retiré : les deux enveloppes (`MODE_ALLEGE`, `MODE_MINIMAL`) sont en place et
  inertes tant que l'hôte sert.
...
- **Vu** : 2026-09-20 · **Mesure** : collecte **647 s** (7,2× le budget), charge **15,82 au début → 81,67 à la fin** ;
  ...
  `gzip -dc` des 9 archives (dont Tracky **172 Mo**), `du` de `/opt` (51 s pour 12 / 21), `find / -maxdepth 5`.
- **QUOI** : le budget de 90 s et la charge de 2 ont été écrits pour une machine qui a ses deux cœurs (VPS-M27 :
  *« l'audit est la charge la plus lourde de la machine »* — à pleine capacité). Quand l'hôte en retient 88 %, la
  même collecte prend dix fois son temps **et** le prend à la production. Un audit qui pèse autant sur ce qu'il
  mesure ne mesure plus rien (VPS-M12), et ses `nice -n 19 ionice -c3` n'y changent rien : le steal se moque de la
  priorité de l'invité.
- **Correctif** : le steal est mesuré sur **2 s** (`/proc/stat`) avant la section 1 ; **≥ 50 % → `MODE_ALLEGE=1`** :
  relecture des archives **> 50 Mo**, parcours de `/opt` et balayage par contenu **NON FAITS — et dits**
  (`mode_allege_dit`, jamais un vide muet : VPS-M02) ; le BUDGET imprime le steal de départ dans les deux cas. Seuil
  sur le **steal**, pas la charge : une charge haute avec du CPU servi est justement ce que l'audit doit mesurer.
  **Passe 2** (script entier, valeurs par défaut) : *« MODE ALLEGE (steal 89 % au depart) : le parcours de /opt NON
  FAIT ce passage »* ✓, `tracky_prod_…sql.gz` *« NON RELUE »* ✓, sections 1–3 en 90 s au lieu de 154 ✓, **413 s au
  total** — ⚠️ **mais charge 8,90 → 68,00** : les `gzip` n'étaient qu'une part ; le reste est la **masse de forks**
  (`ps`, `awk`, sous-shells) d'une collecte de 4 600 lignes sur une VM qui ne sert qu'un cycle sur dix. *Le 1ᵉʳ jet
  réduit la durée, pas la pression.* 2ᵉ jet : **mode minimal** au-delà de 80 % de steal.
- **`aNePasFaire`** : ❌ ne pas relever le budget « parce que la machine est lente » (VPS-M23 : un délai de garde égal
  au budget qu'il protège) ; ❌ ne pas sauter les sections **de lecture** (sar, journaux, timers) : elles coûtent des
  lectures de fichiers, pas du CPU ; ❌ ne pas lire un passage en mode allégé comme un passage complet — les clés
  `/opt`, intégrité des grosses archives, VPS-030 « balayage » y sont **NON MESURÉES**.

### VPS-M106 — Sous disjoncteur, quatre blocs jugeaient un vide : « la production est injoignable », « 38 disparus », « 38 certificats orphelins », « −1 sans sonde »

- **Domaine** : méthode · **Gravité** : 2 · **Statut** : `APPLIQUE` (2026-09-20 — gardes posées, rejouées en passe 2)
- **Vu** : 2026-09-20 · **Mesure** : passe 1, disjoncteur armé à 11:22:23 → section 4 : *« 🟠 DISPARU(S) depuis le
  manifeste du 2026-09-17 : capcom6-mysql, … , vizyo-verify-postgres »* (**les 38**), *« ✅ 0 domaines routes »*, *« 🔴
  PERSONNE ne publie 80/443 — la production est injoignable »* (elle répondait **200** deux sections plus loin),
  *« 38 certificats, dont 38 sans service vivant »*, `bash: line 1204: [: : integer expression expected` ; section 7 :
  *« 1 conteneurs toutes les  »*, *« 0 invocations/min »*, *« -1 SANS AUCUNE SONDE »*.
- **QUOI** : VPS-M02 à la puissance quatre — chaque bloc avait son dénominateur (VPS-M08/M22) mais aucun ne testait
  que sa **source** avait rendu quelque chose : une liste vide passée à `comm -23` rend « tout a disparu » ; un
  `grep` sur rien rend son `||` ; un `awk` sur rien rend `tot-s = -1`. Le disjoncteur (VPS-M104) a fabriqué d'un
  coup le cas que chacun de ces blocs n'avait jamais rencontré.
- **Correctif** : liste courante vide → *« comparaison NON FAITE, ne PAS lire 38 disparus »* ; `INSPECT_JSON` vide →
  *« routage, détenteur de 80/443 et certificats orphelins NON MESURÉS »* (`NB_ETIQ=0`, `ROUTES=""`, plus d'erreur
  `[ ]`) ; cadences vides → *« healthchecksParMin NON MESURÉ »*. **Passe 2** : les trois phrases imprimées, aucun
  verdict faux ✓.
- **`aNePasFaire`** : ❌ ne pas « corriger » en cachant le bloc : le vide doit rester visible et nommé ; ❌ ne pas
  reporter au manifeste un `conteneursListe` vide (le passage suivant comparerait à du vide) — reporter celui du
  dernier manifeste **en le disant**.

### VPS-M105 — Le correctif du matin (M104) était faux : `timeout … command docker` rend 127 partout, et il n'avait jamais tourné

- **Domaine** : méthode · **Gravité** : 2 · **Statut** : `APPLIQUE` (2026-09-20 — banc sur la machine + passe 2)
- **Vu** : 2026-09-20 · **Mesure** : banc isolé (A) : `timeout 5 command docker version` → *« timeout: failed to run
  command ‘command’: No such file or directory »*, **rc=127** ; la première version de la fonction `docker()` (M104,
  écrite à 10:5x, jamais exécutée) aurait donc rendu **127 sur chacun des 31 appels**, avalé par les `2>/dev/null` —
  31 vides silencieux fabriqués par le correctif écrit pour les nommer.
- **QUOI** : `command` est un **builtin** du shell ; `timeout` lance un exécutable. VPS-M35 à l'identique — *« essayer
  les branches ne remplace pas essayer le montage »* — sauf qu'ici même la branche n'avait pas été essayée : la
  collecte du jour avait été produite **avant** le correctif, et la session s'est arrêtée là.
- **Correctif** : `DOCKER_BIN=$(type -P docker)` résolu une fois (`type -P` ignore les fonctions), `[ -n "$DOCKER_BIN" ]
  || return 127`. **Banc** : (B) `docker version` 2,7 s rc 0 ; (C) disjoncteur sur `/bin/sleep 30` avec
  `DOCKER_TIMEOUT=1` : 1ᵉʳ appel **124 en 1 099 ms**, 2ᵉ **124 en 97 ms**, témoin posé puis retiré ; (D) `docker ps -q`
  → 38 en 18,2 s ; (E) 0 client restant. Le témoin `/tmp/audit-vps-docker-disjoncte.$$` est retiré par le trap EXIT.
- **`aNePasFaire`** : ❌ ne jamais compter un correctif « écrit » comme « posé » : il est **programmé**, et il porte le
  risque d'une modification jamais exécutée (règle de VPS-M35, 3ᵉ rappel).

### VPS-M104 — Le collecteur écrit pour dénoncer VPS-016 lançait lui-même 31 clients `docker` sans `timeout`, et l'un d'eux l'a bloqué

- **Domaine** : méthode · **Gravité** : 2 · **Statut** : `APPLIQUE` (2026-09-20 — fonction `docker()` + disjoncteur, réparée par M105, rejouée en passe 1 et 2)
- **Vu** : 2026-09-20 (session du 19/09 en rattrapage, 10:36:57 UTC) · **Mesure** : `docker system df` (section 4,
  « capture unique », VPS-M53) **n'a pas rendu la main en plus de 3 min** sur une machine à charge 13–36 et steal
  80–90 % ; la collecte entière s'est arrêtée derrière lui **sans une ligne** ; arrêtée proprement à 10:49:44 (parent
  `bash -s` 3273662 puis client 3277334) ; `dockerd` a journalisé *« Handler for GET /v1.52/system/df returned error:
  context canceled »* à 10:49:44. Passe 1 : `docker system df` **90 394 ms** (borne 90) ; passe 2 : **45 300 ms**
  (borne 45) ; `docker ps -q` 18–21 s.
- **QUOI** : un `docker` qui ne rend pas la main est **exactement** la classe de VPS-016 (7 sur 7) — et si la session
  SSH qui porte la collecte se ferme avant lui, le collecteur devient la 8ᵉ. Trente et un appels, aucun borné,
  dans le script qui explique depuis le 05/08 pourquoi il faut les borner. La règle vivait dans la procédure (20/08,
  pour les *autres* agents), pas dans le code.
- **Correctif** : fonction `docker()` : `timeout -k 5 "$DOCKER_TIMEOUT" "$DOCKER_BIN" "$@"` (45 s par défaut,
  surchargeable par l'environnement) **+ disjoncteur** — fichier témoin (une variable posée dans un `$(…)` ne
  remonte pas au parent) : au premier 124/137, tous les appels suivants rendent 124 **immédiatement** et l'écrivent
  dans `$ERRBUF` par son chemin (les `2>/dev/null` locaux l'auraient avalé) ; section 4 et BUDGET impriment
  *« DISJONCTEUR docker ARMÉ »* ; la durée du premier appel est imprimée. ⚠️ `timeout` tue le **client** ; la requête
  côté `dockerd` suit son cours — lecture seule, aucun conteneur touché.
- **`aNePasFaire`** : ❌ ne pas remonter `DOCKER_TIMEOUT` « pour que ça passe » : 90 s ont été choisies **une fois**, en
  passe 1 du 20/09, pour ne pas vider les sections 4–5 sur le premier appel, et le choix est écrit dans le rapport ;
  la valeur du script reste 45 ; ❌ ne pas lire « disjoncté » comme « rien » — c'est VPS-M106.

### VPS-M103 — J'ai lu l'effet de V28 dans une série qui a un cycle diurne : les passages courts du soir ont été attribués au cœur rendu

- **Domaine** : méthode · **Gravité** : 3 · **Statut** : `A_TRAITER` (règle écrite, à appliquer ; piste collecteur = angle mort n° 8 du 17/09)
- **Vu** : 2026-09-17 · **Mesure** — `trip_automation_runs`, durée par passage (min) :
  ...
  | 09-15 | 1 → 2 (17 h 20) | 14 | 11 | 13 | 42 | 38 | **56** | **54** | **51** | **57** | **48** | 34 | 13 | 29 | 15 | 16 | 10 | 5 | 6 | 10 |
  | 09-16 | **2** → 1 (12 h 41) | 23 | 13 | **54** | **55** | **55** | **53** | 38 | 44 | **47** | **56** | 42 | 19 | 8 | 4 | 8 | 1 | 10 | 1 | 8 |

  **Le 09-16 de 07 h à 12 h, à DEUX cœurs, rend 54 · 55 · 55 · 53 ; le 09-14 de 08 h à 11 h, à UN cœur,
  56 · 52 · 55 · 59.** Le nombre de cœurs ne se lit nulle part dans cette table ; l'heure de la journée,
  partout. Le rapport du 16/09 (§2, §3) comparait 15 h–17 h (34 · 13 · 29) à 18 h–03 h (15 · 16 · 10 · 5 …)
  et y lisait *« le coût du cœur rendu se lit dans l'application »*. La roadmap du centre d'alerte porte la
  même lecture (*« 7 passages > 45 min le 14/09 … tant qu'il dure, la garde de T29 n'a de fenêtre
  qu'entre :35 et :42 »*).
- **QUOI** : comparer une valeur *avant* et *après* un geste sur une grandeur qui a un **cycle**, sans
  aligner les heures. VPS-M69 (débit lu à une seule heure) et VPS-M71 (jour ouvré contre dimanche) sont
  les deux formes précédentes ; celle-ci est la troisième : **avant/après un geste = jour/nuit**, dès que
  le geste tombe à l'heure où la série décroît d'elle-même.
- **Ce que ça change** : (1) **V28 n'a un effet mesuré que sur `deploy.sh`** (226 / 292 s à deux cœurs
  contre 303–585 / 1 916 s) — et la marge (`sar` 39 → 87 % *à heure égale*) est réelle ; (2) **les passages
  > 45 min qui débordent sur le suivant sont un fait de chaque jour ouvré, 08 h–15 h, quels que soient les
  cœurs** — sujet de **volume** (trajets clos de jour × analyses IA), donc applicatif ; la fenêtre
  HH:42–HH:46 de `deploy.sh` (TRK-077) est prise **toute la journée**, pas seulement pendant un incident.
- **`pourquoiInvisible`** : le geste (17 h 20) est tombé à l'heure où la série décroît naturellement ;
  l'avant et l'après étaient dans la même collecte, et la coïncidence ressemblait à une preuve. Rien dans
  le collecteur n'aligne une série horaire sur la même heure du jour précédent (`positions` a J-7 ; les
  autres n'ont que « la veille »).
- **QUOI FAIRE** : avant d'écrire *« avant → après »* sur une série horaire, **comparer la même heure du
  jour précédent**. *Règle : un geste se mesure à heure égale, ou pas.* Piste collecteur : pour toute série
  horaire, imprimer *« même heure, J-1 »* à côté de *« relevé précédent »*.
- **`aNePasFaire`** : ❌ ne pas retirer de VPS-016 le journal T33 — `deploy.sh` n'a pas de cycle diurne, son
  avant/après tient. ❌ Ne pas écrire « V28 n'a servi à rien » : la marge est mesurée à heure égale sur
  `sar`. ❌ Ne pas corriger la roadmap du centre d'alerte à sa place : le signaler (fait, rapport du 17/09).
...
- **Domaine** : méthode · **Gravité** : 3 · **Statut** : `APPLIQUE` (2026-09-17 — la règle a été tenue un passage entier ; la garde outillée reste un angle mort)
- ✅ **Vu : 2026-09-17 — RÈGLE TENUE** : 5 vérifications en marge, toutes sous `nice -n 19 ionice -c3
  timeout 20`, aucun `find` au-dessus d'un dossier nommé (le seul : `/var/backups/vizyo-verify -maxdepth 1`),
  sur une machine déjà à 1,8–4,5 de charge (VPS-016, 6ᵉ). *Rechute = rouvrir.*
...
- **Domaine** : méthode · **Gravité** : 3 · **Statut** : ✅ `APPLIQUE` le 2026-09-15 (banc sur la
  machine, 1 s, une requête par base Tracky)
- **Vu** : 2026-09-15 · **Mesure** : le rapport du 14/09 a écrit *« depuis l'import de dimanche,
  la démo ne produit plus aucune analyse de trajet (`trip_analyses` 0 arrivée sur 24 h contre
  121) — voulu ou effet de l'import ? »*, et l'a versé dans V27 comme **question produit**. Lecture
  du 15/09 sur `tracky_demo` : **`trip_automation_settings.enabled = false`** (réglé le **09-07
  14 h 45**), **`lastRunAt` NUL** — la démo **n'a jamais analysé un trajet**. Ses 11 753
  `trip_analyses` (max `computedAt` 09-12 22 h 48) sont **copiées de la production par l'import**
  du dimanche, avec leurs horodatages de production. Les « 121 arrivées » d'hier étaient des
  lignes importées dont le `computedAt` de *production* tombait dans la fenêtre de 24 h ; le
  « 0 » d'aujourd'hui, l'absence d'import depuis. **Ni activité, ni arrêt : un miroir.**
- **QUOI — la cause** : la section 5 mesure un débit par `count(*) WHERE computedAt > now() -
  24 h` sur toute base — y compris une base **reconstruite par import**, où la colonne porte la
  date de l'événement **dans la source**, pas la date d'écriture ici. *C'est VPS-M64 / VPS-M69 une
  fois de plus : une grandeur lue sans savoir qui l'écrit.* Et le rapport a ajouté une étape :
  il a nommé une **cause** (« depuis l'import ») sans lire le réglage qui, en une ligne, disait
  que rien n'avait jamais tourné.
- **`pourquoiInvisible`** : l'angle mort n° 4 du 14/09 le disait presque — *« lire le réglage
  d'automatisation du conteneur »* — mais visait `docker inspect` (variables d'environnement), là
  où le réglage vit **en base** (`trip_automation_settings`, un interrupteur maître). Le bon
  endroit n'était pas l'environnement du conteneur : c'était la table que le code lit.
- **QUOI FAIRE** — **`APPLIQUE` le 2026-09-15** : la section 5 imprime, pour chaque base qui porte
  `trip_automation_settings`, **`automatisation des trajets : ✅ ACTIVE (cadence, fenêtre, récits)
  — dernier run …`** ou **`⏸ COUPEE (enabled=false, réglée le …) — dernier run JAMAIS`**, suivie de
  *« les `trip_analyses` de cette base ne sont PAS produites ici : si elles arrivent, elles sont
  importées avec les horodatages de leur source »*. **Banc** : prod `ACTIVE (hourly, h=2, fenêtre
  26 h, récits=f) — dernier run 2026-09-15 01:50 UTC` ; démo `COUPEE (réglée le 2026-09-07 14:45)
  — dernier run JAMAIS`. Coût : une requête par base Tracky.
- **`aNePasFaire`** : ❌ ne pas « corriger » en excluant la démo de la section 5 — ses `positions`
  et `trips` sont, eux, produits ici (le rejeu) et se mesurent ; ⚠️ ne pas lire « ACTIVE » comme
  « fonctionne » : `lastRunAt` est la mesure, `enabled` n'est que l'intention (VPS-M87).

### VPS-M100 — Le verdict de la copie hors-site ne lisait jamais le statut, et il a imprimé « ECHEC … à jour » sur la même ligne

- **Domaine** : méthode · **Gravité** : 2 · **Statut** : ✅ `APPLIQUE` le 2026-09-15 (banc sur la
  machine, < 1 s, coût zéro)
- **Vu** : 2026-09-15 · **Mesure** : section 11, bloc « Copie hors-site », collecte du 15/09 :
  `vizyo-verify ECHEC 21 h a jour (15 copies locales)`. Le fichier d'état portait `statut: ECHEC`
  et `pairesCopiees: 0` depuis le **09-14 04 h 30** ; le verdict (`a jour` / `⚠️ PERIMEE (> 48 h)`)
  ne jugeait que **l'âge de `horodatage`** — la date de la *tentative*. Le rapport du 14/09 a
  publié VPS-037 « inchangé ». **Le copieur avait dit ÉCHEC ; le collecteur l'a affiché et
  contredit sur la même ligne** (VPS-043).
- **QUOI — la cause** : **VPS-M84 sur huit colonnes** — deux verdicts pour la même question, et
  c'est le rassurant qu'on lit. Le bloc avait déjà été corrigé une fois (2026-08-07 : *« la
  fraîcheur d'une COPIE ne peut pas dépasser la fraîcheur de ce qu'elle copie »*, plafonnement par
  la source) — pour l'autre moitié du même défaut : **il jugeait la fraîcheur d'un artefact et
  jamais la réussite de ce qu'il atteste**. Le statut était *affiché* depuis le premier jour ;
  il n'entrait dans aucun verdict.
- **`pourquoiInvisible`** : depuis le 08-07, le copieur n'avait **jamais échoué** — `OK` 38 jours de
  suite, donc les deux verdicts concordaient. Le défaut n'a existé qu'au premier `ECHEC`, et il a
  été vu au passage suivant parce que le **statut** était imprimé à côté (leçon VPS-M02 tenue :
  crier, pas se taire). *C'est VPS-M98 mot pour mot : un défaut qui n'existe que dans le cas
  qu'on n'avait pas encore vu.*
- **QUOI FAIRE** — **`APPLIQUE` le 2026-09-15** : `statut != OK` → verdict **`🔴 ECHEC — RIEN de
  copie a cette tentative`**, puis **`detail` du copieur et `pairesCopiees`** imprimés, et la
  phrase *« la sauvegarde produite depuis la dernière copie RÉUSSIE n'existe qu'ici, sur le disque
  qu'elle protège »*. L'âge ne juge qu'**après** un `OK`. **Banc** : `vizyo-verify ECHEC 22 h
  🔴 ECHEC — RIEN de copie a cette tentative (15 copies locales)` + `detail : téléchargement de
  verify-db_20260830-… en échec (paires copiees : 0)`.
- **`aNePasFaire`** : ❌ ne pas retirer l'âge de la ligne « pour simplifier » : un `OK` de 60 h est
  aussi une panne (le poste éteint), et c'est l'âge qui la voit. ⚠️ **Et ne pas croire que `OK`
  suffit** : `OK` avec `pairesCopiees 0` veut dire « rien à copier », ce qui est vrai un jour
  sans sauvegarde — le bloc du 08-07 (plafond par la source) reste le garde de ce cas-là.

### VPS-M99 — Le bloc « âge par dossier » ne connaissait que `.gz` et `.gpg`, et il a déclaré vide un dépôt de quatorze dumps chiffrés

- **Domaine** : méthode · **Gravité** : 2 · **Statut** : ✅ `APPLIQUE` le 2026-09-14 (banc sur la
  machine, 1,7 s, coût zéro)
- **Vu** : 2026-09-14 · **Mesure** : la section 11 a imprimé **`vizyo-conductor-distant AUCUNE
  SAUVEGARDE`** le **13/09** (ligne 1316 de la collecte archivée) **et** le 14/09, sur un dossier
  qui portait **7 puis 14** fichiers `*.sql.gz.age` (dumps chiffrés de Vizyo Conductor, deux
  instances, reçus chaque nuit à 01 h 00–01 h 08 UTC par `recevoir-dump`, rétention 14). Le bloc
  filtrait `-name '*.gz' -o -name '*.gpg'` : un `.age` lui était invisible, donc le dossier
  « vide ».
- **QUOI — la cause** : **VPS-M88, un cran plus bas.** Le 07/09, la liste blanche de **noms** de
  dossiers avait été retirée de ce bloc (*« on énumère TOUS les dossiers »*) ; la liste blanche
  d'**extensions** restait, et elle décidait du même « rien à signaler ». *C'est la cinquième fois
  de cette famille — VPS-M88, M91, M97, M98, M99 : un contrôle dont le champ est fixé par une
  chaîne que rien ne garantit.*
- **`pourquoiInvisible`** — et c'est la partie qui compte : **le collecteur avait vu, et il avait
  dit.** La ligne était dans la collecte, deux passages de suite. Le rapport du 13/09, qui a fouillé
  `auth.log`, les clés, les `useradd`, `docker inspect` pour VPS-042, **n'a pas lu cette ligne** :
  « AUCUNE SAUVEGARDE » sur un nom inconnu se lit *« un dossier vide de plus »*, au milieu de onze
  lignes semblables. Et le même rapport avait trouvé `recevoir-dump` sans rapprocher ce script de
  ce dossier — **deux blocs de la même collecte portaient chacun la moitié du fait** (VPS-M84).
  🔑 *Un bloc qui énumère « tout » sans qualifier chaque ligne produit une liste où le fait neuf
  est indiscernable de l'habituel.*
- **Le correctif** (banc sur la machine) : `*.age` ajouté aux **quatre** `find` (bloc + second
  volet) ; le **propriétaire** du dossier est imprimé quand il n'est pas `root` (*« dépôt écrit par
  « conductorbk » — copie d'une AUTRE machine, VPS-042 »*) ; et le bloc « dossiers non réclamés »
  gagne une **troisième lecture** — *dépôt d'une autre machine : aucune base ici à couvrir, sa
  fraîcheur mesure la santé de l'expéditeur*. Banc : `vizyo-conductor-distant 1 h · a jour ·
  14 copies · 424K · prd-vizyo_conductor-20260914-030607.sql.gz.age [depot ecrit par « conductorbk »]`.
  **Coût : zéro commande de plus.**
- **`aNePasFaire`** : ❌ ne pas ajouter ce dossier à la **couverture** des bases (il n'y a aucune
  base de cette machine derrière) ; ❌ ne pas retirer la ligne « non réclamé » — elle est vraie,
  elle a désormais sa raison à côté ; ❌ ne pas « tester l'intégrité » d'un `.age` : la clé n'est
  pas ici, et c'est voulu. ⚠️ **Et ne pas croire que la liste d'extensions est désormais
  complète** : elle ne l'est pas, elle ne le sera jamais — le propriétaire non-root est le
  discriminant qui ne dépend pas d'une extension.
...
- ✅ **Vu : 2026-09-14 — le garde a servi pour le 4ᵉ passage** : `/opt ≥ 4,5 Go ⚠️ SOMME PARTIELLE
  (19 / 20 sous-dossiers), en 44 s`, 🔴 « ne se compare pas », un seul manquant (`maalem`,
  12 011 ms). Dénominateur **20** (19 hier, 18 le 09-09) : 🆕 `/opt/tracky-deploiements` (8 Ko, le
  journal de T33) — *le dénominateur bouge sans qu'une ligne le dise, VPS-M92 hors conteneurs.*
...
- 📏 **Vu : 2026-09-21 — LE DIMANCHE 09-20 EST COMPLET : `MOVING` 7,6 %, `INSERTED` 18,4 %.** Série des jours complets :
  ven 09-18 **24,8 / 33,5 %**, sam 09-19 **19,8 / 28,2**, dim 09-20 **7,6 / 18,4** (75 883 lignes) ; jeu 09-17 tronqué
  (29,2 / 33,2, à ne pas comparer), lun 09-21 partiel (2,9 / 12,7). Hier à 11 h le dimanche partiel disait 10,6 / 20,4 —
  *le bord « en cours » surestime toujours `MOVING` (les heures de nuit manquent encore)*. Le dimanche 13/09 n'ayant pas
  été mesuré (passages manqués), **09-20 devient le point de référence « dimanche »** pour le 27/09.
...
- **Domaine** : méthode · **Gravité** : 2 · **Statut** : ✅ **`APPLIQUE` le 2026-09-13** (banc sur
  la machine ; **première comparaison effective le 2026-09-14**)
- ✅ **Vu : 2026-09-23 — PREMIER CONTENEUR NEUF DEPUIS LA POSE : `cdef31-site`, NOMMÉ EN UNE LIGNE (→ VPS-048).** *« 🟠
  NEUF(S) depuis le manifeste du 2026-09-22 : cdef31-site »* + le bloc « RECRÉÉS depuis 24 h » (22/09 13 h 53 min 44). Comptes
  4 = 4, dossiers 13 = 13. ⚠️ Angle mort n° 9 du 23/09 : demain il sera « connu », et plus rien ne redira qu'il n'a ni limite,
  ni sonde, ni copie versionnée.
- ✅ **Vu : 2026-09-21 — LE BLOC A FAIT SON TRAVAIL : UN DOSSIER NEUF, MARQUÉ 🟠, RECONNU EN UNE LIGNE.**
  *« 🟠 dossier(s) de /var/backups NEUF(S) depuis le manifeste du 2026-09-20 : instantanes-chantier-20260915 → à
  reconnaître (VPS-042) »* — c'est **V17** (20/09 13 h 12), annoncé la veille (*« le dossier sera NEUF demain : voulu »*).
  Conteneurs **38 = 38** (mêmes noms), comptes **4 = 4**. Recréés : demo api/web 13 h 40 (V36 a), `tracky-postgres`
  16 h 24 (V29) — les trois hors journal T33, les trois attendus. *Un 🟠 qui se lève en une ligne parce que la passation
  l'avait annoncé : c'est exactement le rendement qu'on demande à ce bloc.*
- ✅ **Vu : 2026-09-14 — PREMIÈRE MESURE EFFECTIVE : `✅ perimetre IDENTIQUE au manifeste du
  2026-09-13 (38 conteneurs, memes noms)`.** Le mécanisme rend — et il rend un « rien » qui se
  démontre (liste contre liste, pas cardinal contre cardinal). ⚠️ **Ce qu'il ne couvre pas s'est
  vu le même jour** : `/opt/tracky-deploiements` (journal de T33, 8 Ko, 09-13 12 h 37) est apparu
  dans le parcours `/opt` (19 / 20 contre 16 / 19) **sans qu'une ligne le dise** — minuteries,
  comptes, dossiers de `/var/backups`, sous-dossiers de `/opt` : même mécanisme, à étendre
  (angle mort n° 2 du 14/09, 2ᵉ report).
...
- 🔴 **Vu : 2026-09-14 — volet (b), 4ᵉ report** : le 🔴 « AUCUNE SAUVEGARDE » sur
  ...
  VPS-M99 vient de montrer le même angle sur un autre dossier : la section 11 raisonne par
  extension et par nom, jamais par mécanisme — un dépôt entrant et une base reconstructible lui
  sont, l'un comme l'autre, « AUCUNE SAUVEGARDE ».*
...
- ✅ **Vu : 2026-09-15 — LA PRÉDICTION DU 14/09 EST TENUE, ET LA RÈGLE DU MÉNAGE EST AFFINÉE.**
  ...
  `avant-20260914-0811-66d286f5` a survécu** — image *construite* le 09-13 20 h 23 (28 h au
  ménage), *étiquetée* le 09-14 08 h 11 (16 h) : le filtre `until` du magasin d'images containerd
  date **l'enregistrement de l'étiquette**, pas le build. *Une image de repli vit donc 24 h après le
  déploiement SUIVANT, pas après sa construction.* Et le disque a fait **+2 Go** malgré −3 images :
  les deux builds du 09-14 (08 h 18 et **11 h 47, jamais déployé**) apportent 1,54 Go de couches
  non partagées (`docker system df` 27,85 → 29,39 Go) — la prédiction « ~52 Go » d'hier comptait
  des tailles annoncées comme du disque, ce que le collecteur interdit lui-même. **Écrit d'avance
  pour le 16/09 00 h 40** : `avant-0811` ×2 (étiquetées 08 h 11 → 40 h) **et `latest` ×2** (11 h 46 /
  11 h 47, inutilisées → 37 h) partent → **27 images**, et **`tracky-api:latest` n'existera plus**
  (le conteneur continue sur `5da87fd…`, étiquetée `avant-…-1140`). *Si 31 images demain : c'est
  que `latest` est utilisée ou que la règle ci-dessus est fausse — les deux se lisent dans le
  nouveau bloc « ce qui TOURNE contre latest ».*
...
- ✅ **Vu : 2026-09-23 — LE SENS INVERSE EST PASSÉ AUSSI : 7 → 1 APRÈS INSTALLATION.** Le bloc disait le 22/09 *« si au
  ...
  rafraîchi 18 h **après** l'installateur → le 1 est « découvert », pas « en retard ».
- ✅ **Vu : 2026-09-17 — LE TEST ÉCRIT LE 16/09 EST PASSÉ** : 3 correctifs de sécurité comptés à 01 h 26
  (mesure valide) → **installés à 06 h 52 min 26** par `apt-daily-upgrade` (polkitd ×3, *« All upgrades
  installed »*), `update-notifier` 64 → 61, cache périmé 73 → 70 dont 3 → 0. Le cycle « compte → installateur →
  zéro » est mesuré de bout en bout pour la première fois.
...
- ✅ **Vu : 2026-09-21 — UN PASSAGE, UN RAPPORT, À L'HEURE : 02 h 23 min 09 (pré-contrôle) → 02 h 23 min 23 (collecte),
  ...
  traîne (19/09 → 20/09) n'a pas eu l'occasion de se rejouer.
- 🔴 **Vu : 2026-09-20 — TROIS PASSAGES, UN RAPPORT : DEUX CAUSES NOUVELLES AU CATALOGUE.** `list_task_runs` :
  **18/09** parti à l'heure (02 h 20 min 43), **interrompu par l'utilisateur** (transcription : 70 messages, *« Request
  interrupted by user »*, 0 rapport, 0 collecte) ; **19/09** non lancé à 02 h 20 (poste endormi), rattrapage à
  **14 h 12 min 04** en collision (VPS-M57), run resté « en cours » **20 h** et collecte le 20/09 à **10 h 36 min 57**,
  bloquée dans `docker system df` (VPS-M104) ; **20/09** non lancé à 02 h 20 (le run du 19 vivait encore), parti à
  **11 h 10 min 29**, **+528 min**, après une veille de 11 h 00 min 38 à 11 h 09 min 22 Z (événements 42 / 107). Pendant
  ces trois jours : la 7ᵉ occurrence de VPS-016 (19/09 18 h 30) et les paliers de l'hôte (22 h 10 → 01 h 10) ont couru
  **13 h** avant qu'une ligne d'audit les nomme — le propriétaire les a trouvés le premier, à 05 h 25, réveillé par
  « Tracky est down ». Le bloc HEURE DE DÉPART a sonné (*« +536 min »*, *« +528 min »*) ; ce qu'il ne sait pas dire :
  **« le passage d'hier n'existe pas »**. Causes au catalogue désormais : poste éteint, quota, veille, **interruption
  humaine**, **rattrapage qui bloque le suivant**. La copie hors-site, elle, a tenu (VPS-037) : *les deux devoirs du
  poste ne dorment pas toujours ensemble.*
- 🟠 **Vu : 2026-09-17 — +56 MIN (03 h 18), LE POSTE DORMAIT DE 23 H 17 À 03 H 15 Z, ET LE BLOC A DIT UNE
  PHRASE FAUSSE.** Le bloc HEURE DE DÉPART posé hier a bien sonné (*« +56 min »*) — mais il affirmait *« la
  copie hors-site (04:30) a probablement été manquée AUSSI »* alors qu'à 03 h 18 elle n'était **pas encore
  due**. *Un retard ne prouve une copie manquée que si l'heure de la copie est passée.* Corrigé : avant
  04 h 30, il écrit *« pas encore due : elle aura lieu seulement si le poste reste éveillé »*. Le réveil
  (source *« Unknown »*) a aussi lancé **les deux audits à la même seconde** (VPS-M57, 3ᵉ).
...
- ✅ **Vu : 2026-09-14 — RETOUR AU RÉGIME : collecte à 02 h 22 min 31, seule, à l'heure habituelle.**
  ...
  les passages d'avant le 10/09. Reste ce que la fiche demande depuis le 09-01 : **rien ne signale
  une ABSENCE** — trois passages ont manqué sans qu'aucune ligne ne l'écrive avant le rattrapage.
...
- **Domaine** : méthode · **Gravité** : 2 · **Statut** : `A_TRAITER` (**rouvert le 2026-09-17** : deux collisions sur trois passages en cinq jours, et la cause est structurelle) — *détecteur posé le 2026-08-22, son dessin d'origine RÉFUTÉ*
- ✅ **Vu : 2026-09-21 — AUCUNE COLLISION : LE CENTRE D'ALERTE A TOURNÉ À 01 h 30, CET AUDIT À 02 h 23.** Le bloc
  « pendant MA collecte » a compté **2 sessions, dont la mienne 1, reste 1** — et ce 1 est le **pré-contrôle V34** lancé
  par cette même session 14 s avant `T_DEBUT` (02 h 23 min 09 ; la fenêtre remonte 15 s). *Même cas que le 22/08 : le
  bloc compte, il ne nomme pas ; la session qui lit le rapport, elle, sait ce qu'elle a lancé.* Pas une collision.
- 🟠 **Vu : 2026-09-20 — QUATRIÈME COLLISION LE 19/09 À 14 H 12 (lue au centre d'alerte), ET UNE FORME NOUVELLE : LE
  RATTRAPAGE QUI TRAÎNE.** Le 19/09 les trois audits (centre d'alerte, VPS, PC) sont partis en rattrapage **à la même
  seconde, 14 h 12** ; celui du centre d'alerte a été interrompu 2 min plus tard, celui du VPS a *« tourné »* **20 h**
  (poste endormi entre-temps) et a collecté le 20/09 à **10 h 36** — pendant que celui du centre d'alerte (parti à
  l'heure à 01 h 09) finissait d'écrire à 10 h 42 et que celui du PC repartait à 10 h 54. Et **le passage VPS de 02 h 20
  du 20/09 n'a pas été lancé** : le planificateur ne double pas un run du même audit encore vivant. Ce passage-ci
  (11 h 18) : **seul** (1 session, la mienne) — les autres avaient fini. *Piste* : `ExecutionTimeLimit` sur les tâches
  Claude, ou une garde « un run de plus de 6 h est abandonné ».
- 🟠 **Vu : 2026-09-17 — TROISIÈME COLLISION, ET ELLE EST STRUCTURELLE.** Après le sommeil du poste
  (23 h 17 → 03 h 15 Z), le planificateur a lancé `audit-centre-alerte-tracky` à **03 h 16 min 25,135** et
  `audit-vps-tracky` à **03 h 16 min 25,356** — 221 ms d'écart, même mécanique que le 13/09 après le quota :
  **tout ce qui est en retard part à la même seconde**. L'autre audit a ouvert **27 sessions SSH** de
  03 h 21 min 30 à 03 h 23 min 10 (mes sections 6 à 12) ; collecte **228 s** (`dockerd` 43,2 %, audit 13,4 %,
  machine à 7,1 % d'inactivité). Les heures déclarées (01 h 09 / 02 h 22, 73 min d'écart) ne protègent de
  rien après un rattrapage groupé : **le remède est le réveil du poste (`WakeToRun`), pas l'horaire.** *Piste
  collecteur* : > 10 sessions d'une autre origine en 2 min → « 🟠 rattrapage groupé probable ».
...
- 📏 **Vu : 2026-09-21 — 149 s (1,7×), 29ᵉ DÉPASSEMENT, MACHINE À PLEINE PUISSANCE : LA RÉFÉRENCE SAINE DU COLLECTEUR
  ...
  `dockerd` 2,7 %. Postes : `/opt` **35 s** pour 21 / 21 (somme complète, 6,0 Go — `maalem` 8,3 s, `vizyo-leads` 8,0 s =
  23 % pour une pile supprimée, VPS-018), `docker system df` **8,1 s**, section 5 **35 s** (t+78 → t+113), section 4
  37 s. **Identique à la passe 3 du 20/09 (125 s à 14 h 09 sur machine saine, 143 s au banc de 19 h 23)** : le collecteur
  vaut ~150 s sur cette machine, et V5 (arbitrer 90 s) reste la seule chose à trancher — *le budget n'est pas un
  chiffre, c'est une décision qui n'a pas été prise*.
- 🔴 **Vu : 2026-09-20 — 647 s (7,2×), 28ᵉ DÉPASSEMENT, ET LA CAUSE EST NOUVELLE : L'HÔTE (VPS-045).** Charge
  **15,82 → 81,67** ; discriminant : `idle` 2,0 % · `nice` 0,31 · `user` 4,9 · `sys` 4,5 · **`steal` 87,7 %** ; *« audit
  22,6 % · `dockerd` 6,6 % · reste 68,8 % »* dont attente 88,1 % — **l'audit valait plus du double de tout le
  `user`+`sys` de la machine** : sur une VM servie à 12 %, il en était la majorité. Passe 2 en mode allégé (VPS-M107) :
  **413 s**, charge **8,9 → 68,0**. *Le budget de 90 s a été écrit pour une machine qui a ses deux cœurs ; il n'a pas
  de sens à 0,2 vCPU, et c'est la première fois que le dépassement vient d'en dehors de la VM.* Le 17/09 (228 s) était
  la 27ᵉ (collision + `dockerd`).
...
- 🔴 **Vu : 2026-09-15 — 182 s (2,0×), 25ᵉ dépassement, MÊME CAUSE EXTÉRIEURE.** Charge 1,69 → 2,92 ;
  ...
  SÉCURITÉ 8 s. Aucune autre session pendant la fenêtre. Trois blocs ajoutés ce passage, coût
  mesuré au banc : < 1 s + 1 s + < 1 s.
- 🔴 **Vu : 2026-09-14 — 189 s (2,1×), 24ᵉ dépassement, MÊME CAUSE EXTÉRIEURE.** Charge 1,44 → 3,56 ;
  ventilation : **audit 16,0 % · `dockerd` 45,4 % · reste 31,0 % · inactif 7,6 %** ; inexpliqué
  après ventilation **22,8 %** (74 % du « reste »). À l'heure (02 h 22), **seul** (aucune autre
  session pendant la fenêtre : la collision de VPS-M57 ne s'est pas reproduite). *Tant que VPS-016
  confisque un cœur, ce chiffre ne mesure que `dockerd` ; l'arbitrage de V5 attend V28.*
