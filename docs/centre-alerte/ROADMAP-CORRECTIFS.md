...
**Au 2026-09-21 (03 h 15 UTC) : 64 faites ·
21 déployées, preuve attendue · 0 commitée · 30 ouvertes** *(115 tâches — dont 3 ouvertes en gravité 1)* — *le compte fait foi dans `app/taches.json`.*
...
| ☐ | **V6** | VPS-037 | 🔴 Second dépositaire de la copie hors-site — **16/09 : le poste dormait à 06:30, pas de copie, 2ᵉ paire de nuit sans copie en 3 jours** (gravité 3 → 2) ; geste immédiat côté poste : `WakeToRun` ; **17/09 : le poste a dormi 23:17 → 03:15 Z, 2 paires sans copie, la 3ᵉ (gravité 1) se joue à 04:30** | » **POSÉE À MOITIÉ 21/09 05:08** — `WakeToRun` sur la copie + tâche `Vizyo-Reveil-Audits` 04:15 (pas sur batterie) ; **mesuré en le faisant** : `STANDBYIDLE = 0`, poste sur batterie 64 %, chaque veille ferme la session → la moitié qui manque est **en admin** : `powercfg /h /type full` + secteur la nuit |
