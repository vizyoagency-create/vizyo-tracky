...
## 🗂️ Tableau de bord — 120 tâches, l'avancement d'un coup d'œil

**Au 2026-09-23 (02 h 45 UTC) : 66 faites ·
19 déployées, preuve attendue · 0 commitée · 35 ouvertes** *(120 tâches — dont 3 ouvertes en gravité 1)* — *le compte fait foi dans `app/taches.json`.*

> 🟢🟠 **LE FAIT VPS DU 23/09 (02 h 29 UTC, À L'HEURE) : LA MACHINE VA BIEN, LE TEST DATÉ DE SÉCURITÉ EST PASSÉ — ET UN SITE
> CLIENT EST ENTRÉ EN PRODUCTION HIER, SA DERNIÈRE VERSION SUR LE SEUL VPS.** Idle **86,07 %** sur le 22/09, 0 client Docker,
> `dockerd` 1,0 %, `/api/health` 30 ms, 34/34/34 (10ᵉ jour ≥ 32), 7/7 sauvegardes, 10/10 archives relues. ✅ **VPS-M74 tranché** :
> l'installateur du 22/09 06:53 a posé **exactement les 7** paquets comptés la veille (`libglib2.0` ×4, `libexpat1`, `libxml2`,
> `rsyslog`) → **7 → 1**. » V37 jour 2/7 (veille 229, pointe 56/h) · » V34 jour 3/7 · VPS-045 jour 3/7 · `alpine` absent (26ᵉ).
> 🟢 **V6** : poste **sur secteur**, audit +7 min, `Vizyo-Reveil-Audits` a **sonné pour la première fois** (04:15, résultat 0) — sur
> un poste déjà éveillé : elle se déclenche, le réveil reste à prouver ; veille prolongée **toujours indisponible**. 🆕 **VPS-048 →
> V38 (gravité 3)** : `cdef31-site` (nginx, `cdef31.vizyoagency.com`, 21 Mo statiques) créé le 22/09 13:53 **sans annonce**, sans
> sonde, sans limite, `/opt` hors sauvegarde — **5 fichiers retouchés sur le VPS à 17:59 UTC, après le dernier zip du poste** :
> `scp -r` + `git init`, 30 s. 🔧 **Méthode ×3** : 🆕 **VPS-M114** (le rapport du 22/09 a publié l'`iowait` sous le nom de *steal* :
> « 0,24 % » pour **2,76 %** réel — verdict intact), 🆕 **VPS-M113** (la charge de départ porte son auteur, bancé), 🆕 **VPS-M115**
> (5 agents `VizyoTracky-*` du poste qui parlent au VPS étaient hors catalogue — `LimitesVitesse` à 02:30 UTC a parlé pendant la
> collecte). 31ᵉ dépassement : 172 s, charge 0,63 → 1,70. **La seule chose à faire en premier : rapatrier `cdef31` (30 s) — puis
> `powercfg /h /type full` en admin.**

> 🔴🟢 **LE FAIT DU 23/09 (centre d'alerte, collecte 01 h 14 UTC) : SEPTIÈME NUIT — LE S21 S'EST TU 2 H 09 PENDANT LA FENÊTRE DE
> COUPE, ET LE SYSTÈME N'A RIEN FAIT DE DANGEREUX, MAIS IL EST REVENU LENTEMENT.** Chute à **18:23:58 UTC** (20:23 Paris, TRK-089 4ᵉ
> épisode en 8 jours), retour vers 20:33. L'interlock a retenu les coupes CDEF31 de 22:00 Paris : **21 `CRITICAL` en 31 min sur 16
> véhicules** (TRK-093 exercé à l'échelle — T79 ; le plafond horaire du socle a regroupé les pushs après 6 min), puis **24 / 24 coupes
> acquittées par TCP**, 23 de **34 à 62 min** en retard. 🆕 **TRK-096 → T82 (gravité 3)** — la dernière coupe est partie **28 min après
> le retour du téléphone** : chaque coupe retenue attend l'échéance de son palier `CUT_BACKOFF_MS` (2 / 5 / 15 / 30 min), rien ne la
> réveille quand l'interlock repasse au vert, et aucune ligne ne dit le retard. La preuve de 21:30 Paris remise **+63 min**, son
> `INDETERMINEE` jamais refermée (T76). **26 / 26 reprises du matin par TCP, 0 SMS.** FV-941-LZ coupé par SMS (socket absente), remis,
> jamais confirmé → `SENT_UNCONFIRMED` **sans une ligne** (immobile, impact nul — TRK-066 (c)). ✅ **T80 : test daté franchi** —
> `email-bloque` a parlé à 07:00:01 ; reste l'adresse de cdef31 avant le 28/09. 📏 T81 : 9 passages écourtés le 22/09. 🔴 **T75 :
> 6ᵉ jour, 147 h sans récit.** **La seule chose à faire en premier : le S21 (T55) — c'est désormais lui, et lui seul, qui a retardé une
> plage de coupe ; puis `claude auth login`.**

> 🟢🔴 **LE FAIT DU 22/09 (centre d'alerte, collecte 01 h 17 UTC — écrit à 05 h 50, le poste ayant dormi entre-temps) : SIXIÈME NUIT
> PROBANTE SUR UNE VM À PLEINE PUISSANCE — ET LE S21 S'EST TU 56 MIN.** **30 reprises / 30** le matin (0,4 → 5,5 s TCP, GT-493-KS par
> SMS en 38,6 s), **27 coupes / 27** le soir (0,4 → 10,8 s, toutes TCP), 0 remise non prouvée, preuve SMS **3 / 3**, `lastTerminalSuccessAt`
> tenu par la preuve de 19:30 sans SMS de secours. Le S21 muet de **22:33:54 à ~23:30** (TRK-089 3ᵉ épisode) : ☑ **T44 FAIT** — l'hystérésis
> a parlé (warn 22:38 → `CRITICAL` 22:39 → rappels 15 min → « 3 contrôles sains » 23:33), réserve écrite (`sim.configuredPresent` null,
> connu). **Et les 10 `CRITICAL` neuves sont toutes nées de cette absence, pour un impact nul** : 🆕 **TRK-093 → T79 (gravité 3)** — 6 refus
> d'interlock + 6 pushs en 50 min sur HM-769-GA et HD-998-XY, **que la garde « hors champ GPS » retenait déjà depuis 20:00** ; la clé
> `interlock|${reason}` porte « dernier ping il y a 266 s » (recalculé toutes les 30 s → espacement, push et socle défaits — la leçon de
> TRK-039), et l'interlock est évalué **avant** les gardes par véhicule → ❌ **contre-preuve de T49**. 📏 **T71 / TRK-083 5ᵉ fois, la première
> MUETTE** : GT-493-KS `kt` +312 ms manqué, SMS payé, ACK par SMS à 38,6 s sous le seuil — la preuve de T71 gagne un 3ᵉ terme. Hors
> coupe-circuit : 🆕 **TRK-094 → T80 (gravité 2, humain)** — **le rapport hebdomadaire de cdef31 n'a jamais été remis depuis le 27/07** (8
> semaines : suppression Resend, rebond dur, `QUEUED` 19 h), 0 ligne ; la sentinelle `email-bloque` du 08/09 doit parler pour la première
> fois **le 22/09 à 07:00 UTC** ; le rebond webhook est muet. 🆕 **TRK-095 → T81 (gravité 3)** — **11 passages d'automatisation sur 24
> écourtés au budget** le 21/09 (4 → 31 véhicules / 34) sur un VPS à 15 % de CPU, sans une ligne ; piste Overpass, à mesurer d'abord. 🔵 3 CDEF31
> non coupés (GPS, TRK-046 voulu), **4 rallumés à la main** (HM-733-GA 6 nuits / 7), TRK-084 pt 4 ×9. 🔴 **T75 : pause 5ᵉ jour, 123 h sans
> récit.** V34 jour 2/7. **La seule chose à faire en premier : `claude auth login` puis « Reprendre maintenant » — puis l'adresse de cdef31
> (T80), avant que le 8ᵉ rapport manqué ne devienne le 9ᵉ dimanche prochain.**

> 🟢🔴 **LE FAIT VPS DU 22/09 (04 h 39 UTC) : LA MACHINE VA BIEN — DEUX FICHES SE FERMENT SUR PREUVE — ET LE POSTE A DORMI SUR
> BATTERIE, EXACTEMENT COMME ÉCRIT LA VEILLE.** Steal **0,24 % de moyenne** sur les 143 relevés du 21/09 (max 2,78), idle
> **87,03 %**, `dockerd` 0,7 % (cumul 0,4 h = ce que 1 % prédit), `/api/health` 38 ms. ☑ **V14 FAIT** (`apt` sonné **01:35** pour
> la 2ᵉ nuit, « MESURE VALIDE » 2/2 par construction → **VPS-033 `APPLIQUE`** ; premier compte de sécurité qui a un sens : **7**,
> installation 06:53, test « 7 → 0 » demain) · **VPS-041 `APPLIQUE`** (`logrotate` **0,4 s**, un seul rotateur, **la rotation
> Docker a purgé 2 troués sur 6** sans geste) · » V37 jour 1/7 (veille 172 sessions, pointe **22/h**) · » V34 jour 2/7 (0 « TUÉ »)
> · VPS-045 jour 2/7 · 34/34/34 émetteurs (9ᵉ jour ≥ 32) · 7/7 sauvegardes datées au journal. 🔴 **V6 : ÉCHEC MESURÉ** — poste
> **sur batterie**, veille critique **03:44:14 Paris**, machine **éteinte 2 h 53** (horloge 01:44Z → 04:37Z), réveil 06:37 **par le
> câble** (1 %) ; `Vizyo-Reveil-Audits` (04:15) **jamais sonné** (refuse la batterie, à dessein) ; **copie hors-site de 04:30 UTC
> manquée et non rattrapée** (`StartWhenAvailable` → prochain passage 23/09) ; audit parti **+137 min**. VPS-037 : **1 paire n'existe
> qu'ici** (gravité 1 à 3). *Hier : « attendre un “réveillé mais session fermée” ». C'est pire : pas de réveil du tout.* 🟠 **VPS-036
> 3ᵉ occurrence** : l'hyperviseur a poussé le 21/09 16:49 **le même tueur de `docker logs` qu'au 28/08, à l'octet** — rien à tuer,
> le lendemain du ticket Hostinger (concomitance, pas identification). 🔧 **Collecteur ×2** : 🆕 **VPS-M111** (`prd` Dispocar
> **1 172 o pour la 10ᵉ nuit** — imprimé une nuit sur deux jusqu'ici, « 8ᵉ nuit NON MESURÉE » hier était un fichier présent
> regardé une fois sur deux ; sous-ligne par instance), 🆕 **VPS-M112** (« le bloc VPS-M110 le dit seul » était **faux** : il ne
> ventile que le jour de pic ; bloc « la VEILLE » + seuil 600/h). 30ᵉ dépassement : 157 s, charge 1,09 → **2,07**. **La seule chose
> à faire en premier n'a pas changé, elle est prouvée : `powercfg /h /type full` dans un PowerShell admin, et le secteur la nuit.**

> 🟢🔴 **LE FAIT VPS DU 21/09 (02 h 23 UTC) : LA MACHINE EST REVENUE À PLEINE PUISSANCE ET Y EST RESTÉE — ET LE COLLECTEUR,
> VENTILÉ POUR LA PREMIÈRE FOIS PAR HEURE, A TROUVÉ 7 774 SESSIONS SSH EN DEUX HEURES DEPUIS LE POSTE.** Steal **0,1–0,3 %**
> sur 34 relevés (13:40 → 02:20), noyau -139, 0 client Docker, `dockerd` 1 %, `/api/health` **33 ms** ; premier passage à
> l'heure et seul depuis le 15/09, **149 s**. **Six preuves venues** : ☑ **V29 FAIT** (`logrotate` **0,6 s** de CPU, un seul
> rotateur, **aucun nouveau trou**, `tracky-postgres` sorti) · » **V32 (a) prouvée** (l'étiquette du 19/09 **a survécu** au
> ménage de 00:40 — 31 images au lieu des 28 prédites sans elle ; (b) attend le `git pull`) · » **V14 1/2** (`apt` sonné
> **01:33**, « MESURE VALIDE ») · ☑ V4 confirmé 10 h après · » V36 (a) lue au journal · » **V34 jour 1/7** (3 « TUÉ » = le
> banc). **VPS-016 → `SURVEILLANCE`**, VPS-045 jour 1/7. 🔴 🆕 **VPS-047 → V37 (gravité 2)** : le 20/09 a porté **7 946
> sessions SSH** (record ×1,9), dont **7 774 entre 14:20 et 16:22 UTC, une par seconde** — `proc/s` **28 → 117**, `user`+`sys`
> **10 → 23 %**, **50 min après** que Hostinger a levé sa limitation « *may be reapplied after sustained usage* ». **Cause
> identifiée dans les transcriptions du poste** : quatre `Monitor` de 30 min à la suite (`while true; do ssh … ; sleep 30;
> done`) — **et `sleep` n'est pas dans le PATH du shell du `Monitor`** : `guetteur.log` porte **1 751 lignes en 30 min**,
> champ `$(date …)` vide ; le 5ᵉ `Monitor` écrit `/usr/bin/sleep` en toutes lettres. *Ce n'était pas une cadence, c'était un
> `sleep` qui ne dormait pas.* Steal inchangé pendant la rafale : le seuil de l'hébergeur est au-dessus, sans qu'on sache où.
> 🔧 **Collecteur corrigé ×3, bancé** : VPS-M108 (10 unités « JAMAIS EXÉCUTÉES » le lendemain du reboot — le journal
> persistant est lu), VPS-M109 (« alpine : PLUS DE 24 h » sur une image que le ménage venait d'épargner — seuil lu dans le
> cron : 72), VPS-M110 (le jour de pic SSH ventilé par origine, clé et heure — angle mort n° 4, 10ᵉ report). ~~**La seule
> chose à faire en premier côté VPS : une ligne dans `CLAUDE.md`**~~ → ✅ **FAIT 05 h 05 Paris sur « Go » du propriétaire**
> (» **V37** : section dans `CLAUDE.md` après V34 + mémoire des agents ; preuve = 7 j sans heure > 600 et le prochain `Monitor`
> relu). » **V6 posée à moitié** (05 h 08 : `WakeToRun` sur la copie + tâche `Vizyo-Reveil-Audits` 04 h 15, pas sur
> batterie) — **mesuré en le faisant** : `STANDBYIDLE = 0` (le PC ne s'endort jamais seul), poste **sur batterie à 64 %**, et chaque
> veille **ferme la session** : la moitié qui rend V6 réel est **en admin** — `powercfg /h /type full` (+3,15 Go) et le secteur
> la nuit. ✅ **Correction** : V32 (b) / V36 (b) sont **EN LIGNE sur le VPS depuis le 20/09 14 h 18** (`fcb1303a`, md5 de
> `deploy.sh` identique) — l'audit du matin les disait « au prochain `git pull` » à tort. **La seule chose à faire en premier
> maintenant : `powercfg /h /type full` dans un PowerShell admin, puis brancher le secteur.**

> 🟢🔴 **LE FAIT DU 21/09 : CINQUIÈME NUIT PROBANTE — SOUS UNE VM À 10 % DE SON CPU, LE MÉCANISME A TENU ET SES INSTRUMENTS
> ONT MENTI.** Journée du 20/09 sous VPS-045 (limitation levée à 13:36) : **31 reprises / 31** le matin (2,6 → 220 s au lieu de
> 0,4 → 7 s, **5 SMS de secours dont 2 pour rien**), **30 coupes / 30** le soir (4 + 26, 0 refus, **HM-733-GA pas rallumé à la main pour
> la première fois en 6 nuits**), **T65 : 4 OK · 1 ⚠️ · 2 🔴 — sur les instruments, pas sur le mécanisme**. 🆕 **TRK-090 → T77 (gravité
> 2)** : la **preuve SMS quotidienne** de 02:30 et 04:30 déclarée « REFUSÉE » puis « **en ÉCHEC — la chaîne SMS est cassée** » (4
> `CRITICAL`, 2 e-mails, 2 pushs) **alors que le téléphone l'avait reçue 38 s après** — 5 échos entrants en base, ignorés parce que
> la soumission avait dépassé le délai client de 10 s (`failed`) et que `verify` ne consulte l'écho que sur un sortant « accepté » ;
> **l'interlock n'a tenu que par un SMS de secours payant** remis à 05:02:57 (sans lui, 26 coupes retenues). 🆕 **TRK-091 → T78
> (gravité 3)** : le **worker RESTORE ré-émet une commande dont le dispatch en ligne est encore en vol** (bail absent à la création,
> numéro lu en mémoire, trame avant la ligne) — 6 commandes, **4 `CRITICAL` « en erreur — intention conservée »** sur des remises
> acquittées 6–17 s après. 📏 **T71 / TRK-083 exercé 3 fois, dont 1 SANS charge** (EP-047-TY CUT 18:00:02, `jt` 43 ms avant la
> trame journalisée → `SENT_UNCONFIRMED` pour une coupe exécutée ; AL-927-QM : SMS payé **7 s après l'ACK**). 📏 **T76 / TRK-089
> 2ᵉ épisode, cause serveur** : 8 `CRITICAL` « aborted due to timeout », 7 pushs, 0 sortie. 📏 TRK-068 ×8 : **personne n'a pu se
> connecter de 05:10 à 06:58**. 🆕 TRK-092 ⚪ (5 messages bruts). 🔴 **T75 non fait : pause 4 j, 99 h sans récit.** ✅ V35 prouvé côté
> application (steal 2 %, passage de 00:45 en 0,6 min), V34 jour 1/7, reboot V4 sans passage interrompu. **La seule chose à faire en
> premier : `claude auth login` sur le poste et « Reprendre maintenant » — puis T77 avant la prochaine nuit sous charge.**

> ✅ **16 h 17 → 16 h 25 UTC — V4 ET V29 FAITES.** Redémarrage sur le noyau **-139** (2 min 40 de coupure, 38/38 revenus
> seuls en 60 s, `tracky-api` même conteneur, swap 0) ; puis `tracky-postgres` recréé avec **14 fichiers de journal par le
> seul rotateur Docker** (arrêt propre par `SIGINT`, `healthy` en 12 s, API reconnectée seule) et **la stanza logrotate à
> `copytruncate` retirée** — plus de second rotateur, plus de trous de NUL à venir. Les 14 jours de journaux d'avant sont
> sauvés dans `/root/`. Du côté VPS, **toute la roadmap du jour est faite** ; restent V30 (Dispocar), V29 bis (ménage des
> anciennes rotations après le 04/10) et le 2ᵉ jet de VPS-M107.
...
### Partie I — centre d'alerte *(40 tâches dans ce tableau ; T34 → T39 et T58 → T75 vivent dans `app/taches.json`)*
...
| ☑ | **T44** | TRK-066 · doc 19 P1-2 | 🔴 Sentinelle Android : hystérésis, bornes d'environnement, fraîcheur par appareil, délai d'envoi séparé du délai de santé | ✅ **FAIT 22/09** — exercée sur l'épisode du 21/09 : warn 22:38 (pas de ligne) → `CRITICAL` 22:39 → rappels 15 min → « 3 contrôles sains » 23:33 ; réserve : `sim.configuredPresent` null (connu), la sortie n'écrit rien (T76) |
...
| ☐ | 🆕 **T77** | TRK-090 · TRK-066 · TRK-018 · T45 | 🟠 **Preuve SMS quotidienne : un délai client de 10 s n'est pas un refus (`queued`, pas `failed`), et l'écho entrant prouve la remise quel que soit le statut du sortant** *(20/09 02:30 et 04:30 : « ÉCHEC — chaîne cassée » deux fois, 5 échos en base, interlock tenu par un SMS de secours payant)* | 🔧 À CODER · **gravité 2** |
| ☐ | 🆕 **T78** | TRK-091 · TRK-083 · T71 · T53 | 🟠 **Le worker RESTORE ne ré-émet pas une commande dont le dispatch en ligne est en vol : bail posé à la création, numéro lu en base, ligne d'attempt puis guetteur puis trame, `P2002` = `warn`** *(20/09 : 6 commandes, 4 `CRITICAL` fausses sur des remises acquittées)* | 🔧 À CODER · gravité 3 |
| ☐ | 🆕 **T79** | TRK-093 · TRK-081 · TRK-089 · T49 · T68 · T76 | 🟠 **L'interlock : une ligne par cause et par quart d'heure, un push par cause et par heure, et « retenue » seulement pour une coupe qui serait partie — un `code` stable dans la raison, pas les secondes du dernier ping** *(21/09 : S21 muet 56 min → 6 `CRITICAL` + 6 pushs en 50 min sur 2 véhicules déjà retenus par la garde GPS ; contre-preuve de T49)* | 🔧 À CODER · gravité 3 |
| ☐ | 🆕 **T80** | TRK-094 · TRK-092 | 🔴 **Le rapport hebdomadaire de cdef31 n'est jamais remis depuis le 27/07 : vérifier l'adresse avec le client, lever la suppression Resend — et un rebond ou un refus à l'envoi doit écrire une ligne** *(8 semaines, 0 ligne ; `email-bloque` du 08/09 attendue le 22/09 07:00 UTC)* | 🤝 HUMAIN · **gravité 2** |
| ☐ | 🆕 **T81** | TRK-095 · TRK-043 · TRK-037 | 🟠 **Un passage d'automatisation écourté sur son budget se compte (`budgetAtteint` / `skippedBudget`), se dit au 3ᵉ de suite (une `DEGRADATION` par épisode) — et le temps Overpass se mesure d'abord** *(21/09 : 11 passages / 24 écourtés, 4 → 31 véhicules / 34, VPS à 15 %, aucune ligne)* | 🔧 À CODER · gravité 3 |
| ☐ | 🆕 **T82** | TRK-096 · TRK-093 · TRK-089 · T79 · T76 | 🟠 **Quand l'interlock repasse au vert, les coupes qu'il a retenues repartent au tick suivant — pas à l'échéance de leur palier de 30 min — et une ligne de sortie dit le retard** *(22/09 : S21 revenu vers 20:33, 18 coupes CDEF31 parties 20 à 29 min après, 34 à 62 min après la plage ; aucune ligne)* | 🔧 À CODER · gravité 3 |

### Partie II — VPS *(39 tâches)*
...
| ☑ | **V4** | VPS-010 | Planifier un redémarrage *(noyau -136 contre -139, 6 services, **3 correctifs de sécurité** ; 16/09 : **débloquée**, uptime 42,3 j ; **17/09 : re-bloquée par V33 — tuer le client AVANT**)* | ✅ **FAIT 20/09 16:17** — 2 min 40 de coupure, noyau **-139**, 38/38 revenus seuls en 60 s, swap 0 |
...
| ☐ | **V6** | VPS-037 | 🔴 Second dépositaire de la copie hors-site — **16/09 : le poste dormait à 06:30, pas de copie, 2ᵉ paire de nuit sans copie en 3 jours** (gravité 3 → 2) ; geste immédiat côté poste : `WakeToRun` ; **17/09 : le poste a dormi 23:17 → 03:15 Z, 2 paires sans copie, la 3ᵉ (gravité 1) se joue à 04:30** ; **22/09 : batterie critique 03:44 → éteint jusqu'au câble à 06:37, réveil jamais sonné, copie manquée et non rattrapée, audit +137 min** | » **POSÉE À MOITIÉ 21/09 05:08** — `WakeToRun` sur la copie + tâche `Vizyo-Reveil-Audits` 04:15 (pas sur batterie) ; **22/09 : ÉCHEC MESURÉ, exactement le scénario prédit** (1 paire sans copie / 7 j) → la moitié qui manque est **en admin, prouvée nécessaire** : `powercfg /h /type full` + secteur la nuit ; preuve attendue demain : réveil 04:15 au journal + **2 paires copiées** à 04:30 |
...
| ☑ | **V14** | VPS-033 | **Fixer l'heure** du rafraîchissement `apt` — **geste confirmé par le code le 14/09** : deux sonneries/jour, la 1ʳᵉ du jour UTC rafraîchit ; 18 % de mesures valides prédits **et** mesurés | ✅ **FAIT ET PROUVÉ 22/09** — posée 20/09 13:12 (drop-in `OnCalendar=01:30` + `RandomizedDelaySec=15m`) ; « MESURE VALIDE » **2/2** : sonné 01:33 le 21/09, **01:35 le 22/09** (cache 01:37:47), par construction ; VPS-033 `APPLIQUE`. Premier compte de sécurité qui a un sens : **7**, installation 06:53 → test « 7 → 0 » le 23/09 |
...
| ☑ | **V29** | VPS-041 | 🔴 **Un seul rotateur pour les journaux de conteneur** — retirer la stanza `logrotate` à `copytruncate` qui perce des trous de NUL *(16/09 : 4ᵉ nuit, 1,63 s de CPU, **6** courants troués — **tous des `-dev-`**, les 3 de production sont sortis par recréation ; ⚠️ `rotate 14` y est posé POUR TRK-035 → compenser par `max-file: 14` sur `tracky-postgres` ; **V28 faite : seule remédiation ouverte sur la classe de VPS-016**)* ; **17/09 : 5ᵉ nuit, 21,9 s de CPU — et la 6ᵉ occurrence de VPS-016 ne visait pas un troué** | ✅ **FAIT 20/09 16:24, PROUVÉ 21/09** — stanza retirée, `tracky-postgres` recréé avec `max-file 14` (`e5fb70fc`) ; **nuit du 21/09 : `logrotate` 0,6 s de CPU, un seul rotateur, aucun nouveau trou, `tracky-postgres` sorti des troués** (V29 bis après le 04/10) |
...
| ☐ | 🆕 **V36** | VPS-046 · 040 | » **(a) FAITE 20/09 13:40** — *démo recréée sur `latest`, 5 migrations jouées, « Import réussi » 13:47 ; (b) `deploy.sh` à coder* — **Recréer la pile de démo sur les images courantes, et faire lancer ce `up -d` par `deploy.sh`** — *20/09 04:07:58 : l'import hebdomadaire a **échoué** — « The column `managedByManagerAt` does not exist » : l'importeur (`tracky-api:latest`, code du 19/09) contre une base de démo au schéma du 13/09 ; la pile démo n'a pas été recréée depuis 3 déploiements, `deploy.sh` ne lance pas le `up -d` de la démo que son compose déclare indispensable ; API démo arrêtée 8 min 35 pour rien, données figées au 13/09* : (a) `docker compose --env-file .env.demo -f docker-compose.demo.yml up -d` quand V35 est levée (ou après le `start` de la pile) ; (b) la ligne dans `deploy.sh` | » **(a) FAITE, (b) EN LIGNE sur le VPS depuis le 20/09 14:18** (même vérification que V32) — preuve : `demo=` au prochain journal, import du 27/09 sans geste |
| ☐ | 🆕 **V37** | VPS-047 · 032 | 🔴 **Plus jamais une boucle du poste qui ouvre une session SSH par seconde** — *21/09 : le 20/09 a porté **7 946 sessions SSH** (record ×1,9), dont **7 774 entre 14:20 et 16:22 UTC, une par seconde** (pic 76/min), toutes depuis le poste ; `proc/s` 28 → 117, `user`+`sys` 10 → 23 %, 50 min après la levée de la limitation Hostinger (« may be reapplied »). **Cause identifiée** : quatre `Monitor` de 30 min à la suite (`while true; do ssh … ; sleep 30; done`) et **`sleep` absent du PATH du shell du `Monitor`** — `guetteur.log` : 1 751 lignes en 30 min, `Mon Sep 21 04:51:32 RDT 2026` vide ; le 5ᵉ `Monitor` écrit `/usr/bin/sleep`* : (1) ✅ identifier ; (2) **une ligne dans `CLAUDE.md`** à côté de V34 — `/usr/bin/sleep` et `/usr/bin/date` dans tout `Monitor`, garde `command -v sleep`, une surveillance du VPS = UNE session qui boucle côté serveur ; (3) ✅ seuil au collecteur (VPS-M110 : 🔴 > 600 sessions/h) | » **POSÉE 21/09 05:05** — section dans `CLAUDE.md` après V34 + mémoire des agents ; preuve : 7 j sans heure > 600 sessions **et** le prochain `Monitor` relu avec `/usr/bin/sleep` · **gravité 2** |
| ☐ | 🆕 **V38** | VPS-048 | **Rapatrier et versionner `cdef31-site`** — *23/09 : site client (nginx, `cdef31.vizyoagency.com`, 3 pages, 21 Mo) créé le 22/09 13:53 sans annonce ; sans sonde, sans limite, `/opt` hors de toute sauvegarde ; 5 fichiers retouchés **sur le VPS** à 17:59 UTC, après le dernier zip du poste (13:34). Geste : `scp -r root@72.62.26.240:/opt/cdef31-vizyoagency D:/www/vizyo-agency/cdef31-vizyoagency` + `git init` (30 s, lecture seule côté VPS) ; plus tard `mem_limit 32m`, pas de sonde.* | 3 | 🤝 HUMAIN |
...
| **16/09** | 55 % | 37 % | 143 s | 33 | V28 prouvée en quatre lectures ; **le repère de repli effacé par le ménage 6 h 25 après le déploiement** *(🆕 VPS-044)* ; le poste dormait, pas de copie |
| **17/09** | 55 % | 55 % | 228 s | 33 | **6ᵉ occurrence de VPS-016** (`docker logs` sans `timeout` depuis le poste, 12:41) ; une lecture d'hier réfutée *(🆕 VPS-M103)* |
| **20/09** *(couvre 18→20)* | 54 % | 43 % | **647 s** | 34 | **L'hôte retient 80–90 % du CPU** *(🆕 VPS-045)* après le 7ᵉ `docker logs` ; l'import de la démo a échoué *(🆕 VPS-046)* ; quatre correctifs au collecteur *(M104 → M107)* |
| **21/09** | 53 % | 32 % | 149 s | 34 | **Pleine puissance retrouvée (steal 0,2 %), six preuves venues (V29, V32 a, V14, V4, V36 a, V34)** ; **7 774 sessions SSH en 2 h depuis un `Monitor` dont le `sleep` ne dormait pas** *(🆕 VPS-047)* ; trois correctifs au collecteur *(M108 → M110)* |
| **22/09** | 53 % | 35 % | 157 s | 34 | **Deuxième jour à pleine puissance (steal 0,24 % moy) ; V14 2/2 → VPS-033 `APPLIQUE` ; VPS-041 `APPLIQUE` (Docker a purgé 2 troués)** ; **le poste a dormi sur batterie : audit +137 min, copie hors-site manquée (V6 : échec mesuré)** ; 3ᵉ tueur de l'hyperviseur *(VPS-036)* ; deux correctifs au collecteur *(🆕 M111 : prd Dispocar lu chaque nuit ; 🆕 M112 : la veille ventilée)* |
| **23/09** | 53 % | 35 % | 172 s | 34 | **À l'heure, sur secteur ; test daté de sécurité passé (7 → 1, les 7 nommés)** ; **un site client (`cdef31-site`) posé et retouché sur le seul VPS** *(🆕 VPS-048 → V38)* ; le *steal* publié la veille était l'`iowait` *(🆕 VPS-M114)* ; deux correctifs de méthode *(M113, M115)* |
...
|---|:--:|---|:--:|---|---|
| **23/09** *(02 h 29 UTC, VPS — à l'heure)* | 🆕 **V38** · **V6** · **V37** · **V34** · **V32** · **V30 (f)** · **V5** · 🆕 **VPS-M113 / M114 / M115** | 🆕 site client hors dépôt · réveil du poste · règle du `sleep` jour 2/7 · garde-fou docker jour 3/7 · repères partis (voulu) · `prd` Dispocar 11ᵉ nuit · budget 31ᵉ · collecteur + catalogue | ☐ **V38 OUVERTE** · » **V6 : la tâche sonne, le réveil n'est pas prouvé** · » V37 · » V34 · » V32 · ☐ V30 · ☐ V5 · 🔧 collecteur (M113) + catalogue (M115) + correction de série (M114) | *(commit du jour — docs)* | 🆕 **V38** : `docker inspect cdef31-site` (sous `timeout 20`) `Created 2026-09-22T13:53:44Z`, `nginx:alpine`, `Memory=0`, pas de `Healthcheck` ; `find /opt/cdef31-vizyoagency -newermt '2026-09-22 14:00'` → 5 fichiers à 17:59 UTC + README 18:01 ; poste : `Downloads\cdef31-vizyoagency.zip` 22/09 15:34 Paris (13:34 UTC), aucun dépôt git. ✅ **VPS-M74** : `apt/history.log` 22/09 06:53:14 → 06:53:44 = 7 paquets ; collecteur *« dont estampillés sécurité : 1 — cache de 0 h, MESURE VALIDE »*. » **V6** : `Get-ScheduledTaskInfo Vizyo-Reveil-Audits` `LastRunTime 23/09 04:15:01`, `LastTaskResult 0` ; journal `System` : aucun `Power-Troubleshooter 1` à 04:15, `Kernel-Power 507` 22/09 23:22 ; `BatteryStatus` `PowerOnline=True` ; `powercfg /a` : veille prolongée indisponible. ✅ **V37** : *« ✅ la VEILLE (2026-09-22) : 229 sessions, heure de pointe 15h=56 »*. ✅ **V34** : `pgrep -a -x docker` vide, témoin `vus=0 tues=0`, 0 « TUÉ » / 24 h. 🔧 **VPS-M114** : `LC_ALL=C sar -u -f sa21` `Average … %iowait 0.24 %steal 2.76`. 🔧 **VPS-M115** : `auth.log` 02:30:02 / 02:30:05 depuis le poste = `VizyoTracky-LimitesVitesse` lancée 04:30:01 Paris. |
| **23/09** *(collecte 01 h 14 UTC, centre d'alerte)* | 🆕 **T82** · **T80** · **T79** · **T76** · **T55** · **T38** · **T75** · **V34** | 🆕 TRK-096 (coupes retenues relancées au palier, pas au retour de la chaîne SMS) · test daté de TRK-094 franchi · TRK-093 2ᵉ exercice (21 lignes / 31 min) · TRK-089 4ᵉ épisode, le premier dans une plage de coupe · nuit 7 : 24 / 24 coupes au retour, 26 / 26 reprises · pause 6ᵉ jour | ☐ **T82 OUVERTE** · ☐ **T80 OUVERTE** *(la sentinelle a parlé ; le geste humain reste)* · ☐ T79 / T76 / T55 / T75 ouvertes | — | `email-bloque` `CRITICAL` 22/09 07:00:01 (suppressed, weekly_report) ; `engine_control_commands` : 23 `CUT` CDEF31 de 20:33:40 à 21:01:50 pour une plage fermée à 20:00, 18 d'entre elles 20 à 29 min après la remise de la preuve de 19:30 (20:33:11) ; `CUT_BACKOFF_MS = [2, 5, 15, 30]` (`schedule-cron.service.ts:67`) ; 0 client Docker résiduel |
| **22/09** *(collecte 01 h 17 UTC, centre d'alerte — écrit 05 h 50, le poste ayant dormi)* | ☑ **T44** · 🆕 **T79** · 🆕 **T80** · 🆕 **T81** · **T49** · **T71** · **T76** · **T38** · **T75** · **V34** | Hystérésis de la sentinelle Android exercée · 🆕 TRK-093 (interlock : clé avec les secondes, gardes après l'interlock) · 🆕 TRK-094 (rapport hebdo cdef31 jamais remis, 0 ligne) · 🆕 TRK-095 (11 passages écourtés au budget, aucune ligne) · nuit 6 probante 27 / 27 et 30 / 30 · pause 5ᵉ jour | ☑ **T44 FAIT** · ☐ **T79 / T80 / T81 OUVERTES** *(T80 gravité 2, humain)* · » **T49 contre-prouvée** *(reste DEPLOYE)* · ☐ T71 *(5ᵉ occurrence, muette — 3ᵉ terme ajouté à sa preuve)* · ☐ T76 *(3ᵉ épisode)* · ☐ T75 *(123 h)* · » V34 jour 2/7 | *(commit du jour — docs)* | ✅ **T44** : journal `tracky-api` 21/09 **22:38:00** « premier contrôle en dégradé » (warn, pas de ligne) → **22:39:00** `CRITICAL` `STALE` 306 s → rappels 22:55 / 23:10 / 23:25 → **23:33:00** « Chaîne Android/SIM de nouveau opérationnelle (3 contrôles sains consécutifs) » ; `/v1/texto/health` 01:26 : `operational: true`, `selection: configured`, `state: ONLINE`, `ageSeconds: 105`, `lastSeen` +60 s entre deux lectures ; réserve : `sim.configuredPresent: null` (`cards: []`, serveur 1.43 — connu). ❌ **T49** : 6 lignes `engine-control-interlock` 22:38:20 → 23:28:50, `subjectKey` de 105–110 caractères **distinct à chaque ligne** (« … 266 s », « … 396 s », « … 706 s », « … 1 486 s », « … 1 616 s », « … 3 296 s »), 6 pushs `coupe-retenue` (12 `SENT`), 0 `GROUPED` — `engine-control.service.ts:1705` `interlock|${reason}`. 📏 **T71** : `wire_logs` GT-493-KS `K;` 05:00:06,654 / `kt` 05:00:06,966 ; `engine_delivery_attempts` TCP `TIMED_OUT` 05:00:22,440 puis SMS `ACCEPTED` 05:00:30 ; ACK 05:00:44 ; 0 `error_logs`. 📏 **T76** : 4 `CRITICAL` watchdog actives, 0 ligne de sortie. 🔴 **T75** : `pauses_agents_locaux` 17/09 08:50 `leveeA` NULL, `ai_usage_logs` 72 h = 0, `rattrapage-recits` 13 « suspendu ». ✅ **V34** : `pgrep -x docker` vide ×4. 🗓️ **T80** : `email_logs` `weekly_report` cdef31 21/09 06:05 `QUEUED` (19 h) — `email-bloque` attendue 07:00 UTC. |
| **22/09** *(04 h 39 UTC, VPS — parti +137 min)* | **V14** · **V6** · **V37** · **V34** · **V30 (f)** · 🆕 **VPS-M111 → M112** | `apt` à heure fixe 2/2 · réveil du poste (preuve **négative**) · règle du `sleep` jour 1/7 · garde-fou docker jour 2/7 · `prd` Dispocar lu chaque nuit · collecteur ×2 | ☑ **V14 FAIT** *(VPS-033 `APPLIQUE`)* · ☑ *(VPS-041 `APPLIQUE` — V29 déjà FAIT)* · » **V6 : ÉCHEC MESURÉ**, moitié admin confirmée nécessaire · » V37 · » V34 · ☐ V30 · 🔧 **collecteur corrigé ×2** | *(commit du jour — docs)* | ✅ **V14** : `systemctl list-timers` `apt-daily.timer` LAST **01:37:40** (sonnerie 01:35:34), `apt-daily.service` succès 01:38:13, cache `/var/lib/apt/periodic/update-success-stamp` 01:37:47, collecteur *« cache de 3 h — MESURE VALIDE »* ; **77 en retard dont 7 sécurité**, installation prévue 06:53:01 (`history.log` : rien depuis le 18/09) → test « 7 → 0 » le 23/09. ✅ **VPS-041** : `CPUUsageNSec=404879000` (0,40 s), 00:00:02 → 00:00:14 ; troués **6 → 4** (`maalem-dev-api` 48 % et `maestroo-dev-api` 21 % **sortis par rotation Docker**), les 4 restants 74/84/84/71 % (84/91/92/83 hier). 🔴 **V6** : journal Windows `System` — `Kernel-Power 42` 03:44:14 Paris, `Kernel-General 1` 06:37:07 *« l'heure est passée de 01:44:15Z »* (éteint 2 h 53), `BatteryStatus` 1 % `PowerOnline=True` à 06:50 ; `Get-ScheduledTaskInfo Vizyo-Reveil-Audits` `LastTaskResult=267011` (jamais), `Vizyo-Verify-Copie-HorsSite` `LastRunTime=21/09 06:30:01`, `NextRunTime=23/09 06:30` ; VPS `DERNIERE-COPIE-LOCALE.json` 21/09 04:30, collecteur *« 1 paire produite depuis la dernière copie réussie »*. ✅ **V37** : `auth.log` 21/09 par heure — max **22** (06 h), total 172 ; `sar -w sa21` `proc/s` 26,9–41,8 (moy 27,6). ✅ **V34** : `pgrep -a -x docker` vide, `journalctl -t docker-orphelins --since -24h` 0 « TUÉ », `dockerd` cumul 0,4 h / 36,3 h. 📏 **V30 (f)** : `ls /var/backups/dispocar-distant` `prd-dispocar-2026092{0,1,2}-*.age` = **1 172 o ×4**, `dev-…-20260922` 49 553 o ; Conductor prd 36 142 / dev 39 077. 🔧 **VPS-M111 / M112** : bancs sur la machine — sous-lignes `instance prd / dev` rendues ; *« ✅ la VEILLE (2026-09-21) : 172 sessions, heure de pointe 06h=22 »*. 🟠 **VPS-036** : `journalctl -u qemu-guest-agent` 21/09 16:49:23 `guest-exec` payload `c25hcD0k…` (1 420 car., = 28/08), 1 occurrence / 14 j. |
| **21/09** *(05 h 05 → 05 h 10 Paris, « Go » du propriétaire)* | **V37** · **V6** · **V32 (b)** · **V36 (b)** | Règle du `sleep` dans `CLAUDE.md` · réveil du poste · *(V32 b / V36 b : déjà en ligne)* | » **V37 posée** · » **V6 posée à moitié** (admin manquant) · ✅ **V32 b / V36 b : correction — EN LIGNE depuis le 20/09 14:18** | *(commit du jour — `CLAUDE.md` + docs)* | ✅ **V37** : `CLAUDE.md` § « 🛑 Une boucle du poste qui parle au VPS = UNE session, et un `sleep` qui dort » (3 formes, garde `[ -x /usr/bin/sleep ]`, cadence relue après 1 min) ; mémoire `claude_monitor_tool_sleep_path.md`. 🔧 **V6** : `Get-ScheduledTask Vizyo-Verify-Copie-HorsSite` → `WakeToRun=True`, `Next 22/09 06:30` (copie du 21/09 06:30:01 `Result 0`) ; `Vizyo-Reveil-Audits` `Ready`, `WakeToRun=True`, `DisallowStartIfOnBatteries=True`, `Next 22/09 04:15` (S4U refusé sans élévation → interactif) ; **mesuré** : `powercfg /q SUB_SLEEP STANDBYIDLE` = `0x0` AC/DC, `Win32_Battery BatteryStatus=1` (décharge) 64 %, `powercfg /a` = S0 + démarrage rapide, hibernation absente → **`powercfg /h /type full` en admin est le geste manquant**. ✅ **V32 b / V36 b** : VPS `git log -1` = `fcb1303a`, `git merge-base --is-ancestor 7c42337c fcb1303a` vrai, `md5sum deploy/vps/deploy.sh` = `cb4aee70ba621a3b100e8169724913c4` des deux côtés, l. 285 `docker inspect --format {{.Image}}`, l. 318 « repère posé », 6 occurrences `up-demo`/`sans-demo`. |
| **21/09** *(02 h 23, VPS)* | **V29** · **V32 (a)** · **V14** · **V34** · **V35** · **V36 (a)** · 🆕 **V37** · 🆕 **VPS-M108 → M110** | V29 prouvée (un seul rotateur) · V32 (a) prouvée (repli survivant) · V14 1/2 · V34 jour 1/7 · VPS-045 jour 1/7 · 🆕 [VPS-047](../vps-audit/REFERENCE-CONSTATS.md) — une session SSH par seconde pendant 2 h depuis le poste (`Monitor` sans `sleep`) | ☑ **V29 FAIT** · » V32 (a) ✅ / (b) attend `git pull` · » V14 · » V34 · ☑ V35 · » V36 · ☐ **V37 OUVERTE** *(gravité 2)* · 🔧 **collecteur corrigé ×3** | *(commit du jour — docs)* | ✅ **V29** : `systemctl show logrotate.service` `CPUUsageNSec=601534000` (0,60 s), 00:00:00 → 00:00:09 ; collecteur *« ✅ un seul rotateur »* ; troués **6**, tous `-dev-`, tous en baisse (48/21/84/91/92/83 % contre 71/45/95/96/97/94), `tracky-postgres` absent ; `/var/lib/docker/containers` 318 → 245 Mo. ✅ **V32 (a)** : `docker images` 02:24 — `tracky-api:avant-20260919-1537-1d1521b2 84cc3fe36db2 3 days ago`, `tracky-web:avant-… 99811d3a61a2`, `alpine 294b683cb724 3 days ago` (46 h) ; cron `until=72h` ; 31 images. ✅ **V14** : `apt-daily.service` succès 01:33:52, cache 01:33:29, *« cache de 0 h — MESURE VALIDE »*. ✅ **V34** : `pgrep -a -x docker` vide 02:23:10, témoin `dernier=02:24:50 vus=0 tues=0`, `journalctl -t docker-orphelins` 24 h = 3 lignes `faux-conteneur` / `tracky-lp` sous `timeout` / `faux-chaine` (banc 13:08–13:09). ✅ **V35** : `sar -u sa20/sa21` steal 0,13–0,99 % (13:40 → 02:20), idle 86–89 %, `/api/health` 33 ms. 🔴 **VPS-047** : `auth.log` `Accepted` 20/09 = **7 946** (7 941 `root` from `82.67.153.51` SHA256:cdd9… ; par heure 14 h 2 563 · 15 h 3 894 · 16 h 1 317 ; par 10 min 14:2x 589 → 16:2x 194 → 6) ; `logind` sessions 47699–47702 : ouverture → fermeture 0,6–1,0 s ; `sar -w` `proc/s` 27,96 (14:00) → 117,44 (15:00) → 27,50 (16:50) ; `sar -u` `user`+`sys` 9,8 → 23,4 % ; transcription `87c19204` : `Monitor` 14:21:03 / 14:51:26 / 15:22:54 / 15:53:45 (`sleep 30/30/20/15`, `timeout_ms 1800000`), 16:17:35 (`S=/usr/bin/sleep`) ; `guetteur.log` **1 751 lignes**, préfixe date vide. 🔧 **VPS-M108** : banc `journalctl -u tracky-backup.service` → `2026-09-20T04:05:08 succès` ×7 unités en 411 ms. 🔧 **VPS-M109** : `grep until /etc/cron.d/docker-image-prune` = 72, âge 46 → « ÉPARGNÉ, 26 h de sursis ». 🔧 **VPS-M110** : le tableau ci-dessus, rendu par le bloc. |
| **21/09** *(01 h 30, centre d'alerte)* | 🆕 **T77** · 🆕 **T78** · **T71** · **T76** · **T38** · **T75** · **V34** · **V35** | TRK-090 (preuve quotidienne « cassée » alors que remise) · TRK-091 (worker RESTORE vs dispatch en ligne) · TRK-083 ×3 dont 1 sans charge · TRK-089 2ᵉ épisode (relais) · nuit 5 probante 30 / 30 · pause 4 j | ☐ **OUVERTES** *(T77 gravité 2, T78 gravité 3)* · ☐ T71 / T76 / T38 / T75 · » V34 jour 1/7 · ☑ V35 prouvé côté application | *(commit du jour — docs)* | Rapport [2026-09-21](./rapports/2026-09-21.md) : 31 reprises / 31 (5 SMS dont 2 inutiles), 30 coupes / 30 ; `sms_logs` : preuves de 02:30 / 04:30 `failed` **et** 5 échos entrants à +38 s ; `engine_delivery_attempts` : 1 seule tentative nº 1 sur les 6 commandes « Unique constraint » ; `wire_logs` EP-047-TY : `jt` 18:00:02,277 avant la trame 18:00:02,320 ; steal 2 %, 0 client Docker, timer actif |
| **20/09** *(16 h 17 → 16 h 25)* | **V4** · **V29** | Redémarrage sur le noyau -139 ; `tracky-postgres` recréé avec 14 fichiers de journal par le seul rotateur Docker, stanza logrotate retirée | ☑ **V4 FAIT** · » **V29 déployée** (preuve : nuit du 21/09 sans nouveau trou, `logrotate` < 1 s) | `e5fb70fc` (compose) | ✅ **V4** : liste de contrôle 16:17:06 toute verte (0 passage, 0 commande, 38/38 `unless-stopped`, 103 min avant la coupe) ; `systemctl reboot` 16:17:23 ; SSH 16:20:03 ; `uname -r` **6.8.0-139-generic**, `reboot-required` disparu ; **38/38 running, 0 starting/unhealthy à 60 s** ; `tracky-api` = `3ec0c6d13241` (inchangé) ; 10 URL 200/301/302/307 ; port 5023, 29 positions / 171 trames en 5 min ; swap 0 Mo. ✅ **V29** : 75 Mo de journaux sauvés ; `docker stop --signal SIGINT` → *« database system is shut down »* 16:23:59 ; nouveau conteneur `cce3585f73e9` `LogConfig {max-file 14}` `StopSignal SIGINT`, `healthy` 12 s, *« was shut down at 16:23:59 … ready to accept connections »* ; `/api/health` 200 ×6, 112 positions / 3 min, 0 erreur Prisma ; `logrotate -d` : 0 `docker/containers`. |
