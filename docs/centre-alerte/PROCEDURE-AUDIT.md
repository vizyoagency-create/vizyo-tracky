...
|---|---|---|---|---|
| 1 | **La preuve SMS quotidienne** est partie à 04:30, 06:30 **et 21:30** (Paris, le passage de 21:30 depuis le 16/09) et vérifiée OK à +15 min | `SELECT to_char("createdAt",'DD HH24:MI') AS h, level, left(message,110) FROM error_logs WHERE source='sms-daily-proof' AND "createdAt" > now()-interval '36 hours' ORDER BY "createdAt";` — **et** le journal du conteneur : `docker logs --since 36h tracky-api 2>&1 \| grep -a "Preuve SMS quotidienne" \| tail -6` (sous `timeout 20`, VPS-041) | trois passages « acceptée » et trois `verdict=OK` par jour, **aucune** ligne `sms-daily-proof` au centre d'alerte | une ligne `INDETERMINE` = on ne sait pas si les SMS partent ; `ECHEC`/`NON_EMIS` = les coupes du soir seront retenues par l'interlock — le dire en 🔴, nommer l'heure. **Nuit du 15 au 16/09 : à 22:00 la dernière remise prouvée avait 37 h → 24 coupes retenues jusqu'à la preuve de 04:30** — c'est ce que le passage de 21:30 empêche |
...
| 7 | **Quelqu'un a été prévenu** — depuis le 16/09, chaque cause qui bloque le coupe-circuit pousse une notification aux super-admins (coupes retenues, preuve KO, téléphone hors ligne, remise en route non confirmée) | `SELECT to_char("createdAt",'DD HH24:MI') AS h, "alertType" AS kind, "subjectKey", status, coalesce(reason,'') FROM notification_deliveries WHERE category='SYSTEM' AND "alertType" IN ('coupe-retenue','preuve-sms','passerelle-sms','restore-non-prouvee') AND "createdAt" > now()-interval '24 hours' ORDER BY "createdAt";` | pour chaque ligne CRITICAL `engine-control-interlock` / `sms-daily-proof` / `sms-gateway-watchdog` / `engine-control-restore` de la journée, **une** notification SENT (ou GROUPED par le cooldown) vers le propriétaire | une cause bloquante sans notification = le chaînon `coupe-circuit.push` est cassé (PUSH_ROLLOUT, préférences, appareil push) — à dire en 🔴 : le système sait mais ne prévient pas |
...
Le rapport reprend ces sept verdicts dans une section « Coupe-circuit — 24 h de preuve » placée **avant** « Chiffres ».
