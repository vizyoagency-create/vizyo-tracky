...
| [TRK-071](#trk-071) | `AI_ROUTER` | **Le repli IA est né inerte : le SECOND compte est à sec aussi** — `Repli IA en échec : claude → gpt (provider_unfunded) — …` | 🔵 **TERRAIN** *(action humaine : recharger)* · **gravité 1** · famille **terrain** *(📏 **13/09 : le TROISIÈME canal — la CLI du poste, `executor = local` — s'est révélé plafonné à son tour** : 80 h sans aucun canal IA ouvert (cf. TRK-069). Anthropic à sec depuis le 03/09 00:13 (**250 h**), dernier appel API réussi le 03/09 00:01:02. 1 occurrence, 05/09 12:23:59. Le repli `claude → gpt` livré le jour même (chantier C3 point 1) a été exercé **six minutes après sa mise en ligne** et n'avait nulle part où aller : Anthropic à sec depuis le 03/09 00:13, OpenAI constaté vide. **Ce n'est PAS un défaut de code** — le routeur bascule, échoue, l'écrit, le date et le classe correctement en `DEGRADATION`. 🔑 **Un mécanisme de secours ne vaut que par la ressource qu'il vise** : livrer une redondance sans provisionner sa cible, c'est livrer une ligne de journal. Action : recharger **au moins un** des deux comptes)* | 2026-09-06 | 2026-09-06 |
