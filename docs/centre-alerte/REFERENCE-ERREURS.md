...
|---|---|---|---|---|---|
| [TRK-051](#trk-051) | *commandes* | **`ACKNOWLEDGED` ne veut pas dire « acquitté par le boîtier »** — la clôture par échéance écrit ce statut dès que la cadence MESURÉE rejoint la cadence demandée, sans aucune confirmation du matériel | 🟠 **CORRECTIF ÉCRIT ET TESTÉ le 26/08, NON DÉPLOYÉ** (option 2, sans migration — branche `fix/trk-051-acknowledged-sans-accuse`) · **gravité 2** *(nouvelle ; **120 `ACKNOWLEDGED` sur 7 j, 2 seulement portent une vraie réponse de boîtier** — les 118 autres sont des `fix_continuous` dont le propre `observedResult` dit « sans accusé de réception du boîtier ». `ackedAt` = **2 sur 5 173**. Cause : `TrackerCommandStatus` n'a aucune valeur pour « a produit l'effet demandé, sans confirmation matérielle », donc PR #111 ([TRK-013](#trk-013), 23/08) a réutilisé la moins fausse — `tracker-fix-mode.service.ts:463`. ⚠️ **Le code protège pourtant `ackedAt`/`ackResponse` exprès pour ne pas truquer la mesure de [TRK-014](#trk-014)** : il a protégé l'instrument et corrompu sa lecture, parce que le champ réutilisé n'est pas celui qu'il a protégé. Coût réel : **les passages du 24 et du 25/08 ont failli refermer TRK-014**. ⚠️ **Corollaire** : le « métronome de 72 échecs/jour » était un artefact d'étiquetage — TOTAL `fix_continuous` plat à 69-79/j sur neuf jours pendant que `FAILED` tombait de 72 à 15. ✅ `collecte.sql` durci le 26/08 (`reponse_boitier` + `total_famille`) ; l'écran admin ne l'est pas)* | 2026-08-26 | 2026-08-26 |
...
**Statut : 🟠 CORRECTIF DÉPLOYÉ le 2026-08-23 (PR #110), NON EXERCÉ** · découvert 2026-08-13
...
**Statut : 🟠 CORRECTIF DÉPLOYÉ le 2026-08-19, NON EXERCÉ** · découvert 2026-08-13
...
**Statut : 🟠 CORRECTIF ÉCRIT ET TESTÉ le 2026-08-26, NON DÉPLOYÉ** (option 2, sans migration) · **gravité 2** · famille **mensonger** · 2026-08-26 · ancien statut : *🔴 NON CORRIGÉ*
