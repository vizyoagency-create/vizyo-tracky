...
@Component({
  ...
  styles: [`
    ...
    .toggle-track {
      display: block; width: 40px; height: 22px; background: var(--border-subtle); border-radius: 22px;
      transition: background .2s; position: relative;
    }
    .toggle-thumb {
      position: absolute; width: 18px; height: 18px; left: 2px; top: 2px;
      background: var(--fg-tertiary); border-radius: 50%; transition: transform .2s, background .2s;
    }
    .toggle input:checked + .toggle-track { background: rgba(16,224,160,.3) }
