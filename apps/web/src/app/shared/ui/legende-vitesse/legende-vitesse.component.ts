 ...
 *
 * Deux dispositions, parce que deux hôtes : la liste verticale du HUD de bureau, la grille
 * à deux colonnes de la feuille mobile. La taille de police se règle par l'hôte avec
 * `--lv-taille` (10 px par défaut, 12 px dans la feuille), pas par une variante de plus.
 */
@Component({
  ...
  template: `
    <ul class="lv" [class.lv--grille]="disposition() === 'grille'" aria-label="Couleurs de vitesse">
...
export class LegendeVitesseComponent {
  ...
  readonly disposition = input<'liste' | 'grille'>('liste');
}
