...
export function createTrackyIcon(speed: number, heading = 0, vehicleType = 'OTHER'): L.DivIcon {
  const color = speedColor(speed);
  const svgContent = getVehicleSvg(vehicleType);
  // Only rotate the arrow (OTHER) type — vehicle icons look better without rotation
  const rotate = vehicleType === 'OTHER' ? `transform:rotate(${Math.round(heading)}deg);` : '';
  return L.divIcon({
    className: '',
    html: `<div style="position:relative;width:44px;height:44px">
      ...
      <div style="position:relative;width:44px;height:44px;border-radius:9999px;display:flex;align-items:center;justify-content:center;background:${color};border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.4),0 0 20px ${color}60">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="${rotate}filter:drop-shadow(0 1px 2px rgba(0,0,0,0.3))">
          ${svgContent}
        </svg>
      </div>
    </div>`,
